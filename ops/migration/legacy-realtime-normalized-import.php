...
require_once dirname(__DIR__, 2) . '/bot/migration/LegacyRealtimeNormalizedImportService.php';
...
try {
    ...
    if ($environment === 'production') {
        ...
        $privateApproval = !empty($config['legacy_realtime_normalized_import_allow_production']);
        if (!$explicitApproval || !$privateApproval) {
            throw new RuntimeException(
                'Production normalized realtime import requires private approval and --allow-production.'
            );
        }
    }
    ...
    $lockHandle = fopen($privateDir . '/legacy-realtime-normalized-import.lock', 'c+');
    if ($lockHandle === false || !flock($lockHandle, LOCK_EX | LOCK_NB)) {
        throw new RuntimeException('Another normalized realtime import is already running.');
    }
