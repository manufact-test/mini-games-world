...
require_once dirname(__DIR__, 2) . '/bot/accounts/MgwIdGenerator.php';
...
require_once dirname(__DIR__, 2) . '/bot/accounts/LegacyAccountOwnershipLinkService.php';
...
try {
    ...
    if ($environment === 'production') {
        ...
        $privateApproval = !empty($config['legacy_account_ownership_link_allow_production']);
        if (!$explicitApproval || !$privateApproval) {
            throw new RuntimeException(
                'Production legacy account ownership linking requires private approval and --allow-production.'
            );
        }
    }
    ...
    $lockHandle = fopen($privateDir . '/legacy-account-ownership-link.lock', 'c+');
    if ($lockHandle === false || !flock($lockHandle, LOCK_EX | LOCK_NB)) {
        throw new RuntimeException('Another legacy account ownership link is already running.');
    }

    $storage = StorageFactory::create($config);
    $service = new LegacyAccountOwnershipLinkService(
        $storage,
        $database,
