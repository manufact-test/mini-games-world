...
try {
    ...
    if ($environment === 'production') {
        ...
        $privateApproval = !empty($config['legacy_opening_balance_import_allow_production']);
        if (!$explicitApproval || !$privateApproval) {
            throw new RuntimeException('Production opening balance import requires private approval and --allow-production.');
        }
    }
    ...
    $lockPath = $privateDir . '/opening-balance-import.lock';
    $lockHandle = fopen($lockPath, 'c+');
    if ($lockHandle === false || !flock($lockHandle, LOCK_EX | LOCK_NB)) {
        throw new RuntimeException('Another opening balance import is already running.');
    }

    $service = new LegacyOpeningBalanceImportService(
        $connection,
        new LedgerWriteService($connection),
        new LedgerIntegrityVerifier($connection)
    );
