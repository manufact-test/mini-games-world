...
$runAccountDataRetention = static function (
    ...
    $summary = (new AccountDataLifecycleService($database, $runtimeStorage, $config))->runRetention();
    $failed = max(0, (int)($summary['deletions_failed'] ?? 0));
    if ($failed !== 0) {
        throw new RuntimeException('Account data retention reported failures.');
    }
    return [
        'ok' => true,
        'deletions_checked' => max(0, (int)($summary['deletions_checked'] ?? 0)),
