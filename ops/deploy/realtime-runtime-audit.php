...
try {
    ...
    if ($environment !== 'staging') {
        throw new RuntimeException('Realtime runtime audit is enabled only in staging.');
    }
    ...
    if (!$router->enabled()
        ...
        || $router->routeFor('realtime') !== RuntimeStorageRouter::DRIVER_DATABASE) {
        throw new RuntimeException('Accounts and realtime DB routing must be enabled before the audit.');
    }

    $storage = StorageFactory::create($config);
    if ($storage->driver() !== RuntimeStorageRouter::DRIVER_JSON) {
        throw new RuntimeException('Global JSON rollback storage must remain active during the realtime audit.');
    }
    ...
    $lockHandle = fopen($privateDir . '/realtime-runtime-audit.lock', 'c+');
    if ($lockHandle === false || !flock($lockHandle, LOCK_EX | LOCK_NB)) {
        throw new RuntimeException('Another realtime runtime audit is already running.');
    }
    ...
    $audit = (new RuntimeRealtimeRepository($config, $router, $database))->auditParity($snapshot);
    $blockers = array_values(array_unique(array_map(
        'strval',
        (array)($audit['blockers'] ?? [])
    )));

    $sourceGameCount = (int)($audit['source_game_count'] ?? 0);
    $databaseGameCount = (int)($audit['database_game_count'] ?? 0);
    $sourceQueueCount = (int)($audit['source_queue_count'] ?? 0);
    $databaseQueueCount = (int)($audit['database_queue_count'] ?? 0);

    $result = [
        'ok' => $blockers === [] && !empty($audit['ok']),
        'read_only' => true,
        'source_game_count' => $sourceGameCount,
        'database_game_count' => $databaseGameCount,
        'database_total_game_count' => (int)($audit['database_total_game_count'] ?? 0),
        'retained_terminal_game_count' => (int)($audit['retained_terminal_game_count'] ?? 0),
        'source_queue_count' => $sourceQueueCount,
        'database_queue_count' => $databaseQueueCount,
        'parity' => $blockers === []
            && $sourceGameCount === $databaseGameCount
            && $sourceQueueCount === $databaseQueueCount,
        'source_fingerprint' => (string)($audit['source_fingerprint'] ?? ''),
        'database_fingerprint' => (string)($audit['database_fingerprint'] ?? ''),
        'sensitive_identifiers_exposed' => false,
        'blockers' => $blockers,
        'report_type' => 'mvp-14.8.2e-realtime-runtime-audit',
