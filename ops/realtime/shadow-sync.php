...
try {
    ...
    if ($environment === 'production') {
        ...
        $privateApproval = !empty($config['realtime_shadow_sync_allow_production']);
        if (!$explicitApproval || !$privateApproval) {
            throw new RuntimeException('Production realtime shadow sync requires private approval and --allow-production.');
        }
    }
    ...
    $lockPath = $privateDir . '/realtime-shadow-sync.lock';
    $lockHandle = fopen($lockPath, 'c+');
    if ($lockHandle === false || !flock($lockHandle, LOCK_EX | LOCK_NB)) {
        throw new RuntimeException('Another realtime shadow sync is already running.');
    }

    $storage = StorageFactory::createJson((string)($config['data_dir'] ?? ''));
    $service = new LegacyRealtimeShadowSyncService($storage, $connection);
