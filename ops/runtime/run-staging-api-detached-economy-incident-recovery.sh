...
fail() {
  local reason="$1"
  printf 'MGW_STAGING_API_DETACHED_INCIDENT_RECOVERY=BLOCKED\n'
  printf 'REASON=%s\n' "$reason"
...
CANDIDATE_LIST="$PRIVATE_DIR/staging-api-detached-incident-receipt-candidates-$LIST_ID.txt"
[[ ! -e "$CANDIDATE_LIST" && ! -L "$CANDIDATE_LIST" ]] \
  || fail 'fresh private candidate list path already exists'
(
  set -o noclobber
  find "$PRIVATE_DIR" -maxdepth 1 -type f \
    ...
    -printf '%T@ %p\n' 2>/dev/null \
  | sort -nr \
  | cut -d' ' -f2- > "$CANDIDATE_LIST"
) || fail 'private incident receipt candidate list could not be created'
chmod 0600 "$CANDIDATE_LIST" \
  || fail 'private incident receipt candidate list permissions are invalid'
...
rm -f -- "$CANDIDATE_LIST" || fail 'private candidate list could not be removed'
CANDIDATE_LIST=''
[[ -n "$RECEIPT" ]] || fail 'exact detached incident read-only receipt was not found'
...
[[ ! -e "$REFRESHED_RECEIPT" && ! -L "$REFRESHED_RECEIPT" ]] \
  || fail 'fresh temporary detached incident receipt path already exists'
...
' "$PROJECT_ROOT" "$ARCHIVED_RECEIPT" "$REFRESHED_RECEIPT" "$INCIDENT_COMMIT"; then
  fail 'archived detached incident receipt could not be safely revalidated and refreshed'
fi

REPORT="$PRIVATE_DIR/staging-api-detached-economy-incident-recovery-$RUN_ID.json"
RUN_STATUS="$PRIVATE_DIR/staging-api-detached-economy-incident-recovery-status-$RUN_ID.json"
for path in "$REPORT" "$RUN_STATUS"; do
  [[ ! -e "$path" && ! -L "$path" ]] \
    || fail 'fresh private detached incident recovery output path already exists'
done

if ! "$PHP_BIN" \
    "$PROJECT_ROOT/ops/runtime/recover-staging-api-mutating-smoke-detached-economy-incident.php" \
  ...
  fail 'exact detached staging API economy incident recovery failed'
fi
...
if (!is_array($d)
    || ($d["ok"] ?? null) !== true
    || ($d["action"] ?? "") !== "staging_api_detached_economy_incident_recovery_passed"
    || ($d["repository_commit"] ?? "") !== $argv[2]
    || ($d["incident_repository_commit"] ?? "") !== $argv[3]
    || ($d["state_revision"] ?? null) !== 3
    || ($d["failed_event_recovered"] ?? null) !== true
    || ($d["detached_economy_rows_removed"] ?? null) !== true
...
echo "MGW_STAGING_API_DETACHED_INCIDENT_RECOVERY=PASSED", PHP_EOL;
...
echo "DETACHED_ECONOMY_ORPHAN_ROWS_REMOVED=true", PHP_EOL;
...
' "$RUN_STATUS" "$CURRENT_COMMIT" "$INCIDENT_COMMIT" "$PHP_VERSION_SAFE" \
  || fail 'exact detached staging API economy incident recovery proof is invalid'
