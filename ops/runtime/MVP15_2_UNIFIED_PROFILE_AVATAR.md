...
The shared legacy `renderUser()` no longer writes `profileName`, `profileAvatar` or `profileDate`. It continues to own Home/Search user presentation until those surfaces are migrated in their own roadmap stages.

## Visible canonical fields

The profile screen renders from the MGW profile response:
...
The avatar DOM owner is `mgw_id`, not Telegram id. Only `http:` and `https:` external avatar URLs are accepted by the browser renderer. No provider SDK is consulted by the profile screen once the canonical profile response exists.

`avatar_storage_key` is intentionally not converted into a URL in this stage because there is no canonical first-party avatar delivery endpoint yet. A future upload/storage-backed avatar must introduce one resolver rather than inventing storage paths in the client.
...
It does not render visible profile name, avatar, MGW id or registration date. This transitional dependency is narrowed further by MVP-15.3+ when economy state is unified.
...
The exact staging fingerprint includes all changed profile runtime files plus the provider-neutral `AccountIdentityService` foundation from MVP-15.1.
...
- no Google credential verification enabled;
