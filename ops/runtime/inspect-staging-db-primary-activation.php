...
if (count($argv ?? []) !== 1) {
    fwrite(STDERR, "This read-only activation inspector accepts no arguments.\n");
    exit(2);
}
...
if ($environment !== 'staging') {
    fwrite(STDERR, "DB-primary activation inspection is staging-only.\n");
    exit(2);
}
...
try {
    ...
    if (!$databaseConfig->enabled()) {
        throw new RuntimeException('DB-primary activation inspection requires an enabled staging database.');
    }
    ...
    $report = (new RuntimePrimaryStagingActivationGuard(
        $projectRoot,
        $config,
        (string)($configFile ?? ''),
        $jsonStorage,
    ...
    fwrite(STDOUT, json_encode([
        ...
        'action' => 'staging_activation_blocked',
