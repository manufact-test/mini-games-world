  ...
  for (const delayMs of RETRY_DELAYS_MS) {
    ...
    try {
      ...
      if (snapshot.authoritative) {
        ...
        if (authoritativeEmptyResponses >= REQUIRED_AUTHORITATIVE_EMPTY_RESPONSES) return response;
      }
    } catch (error) {
    }
  }
  if (authoritativeEmptyResponses >= REQUIRED_AUTHORITATIVE_EMPTY_RESPONSES) return latestResponse;
