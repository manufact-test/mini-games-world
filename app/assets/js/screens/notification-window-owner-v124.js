...
const INPUT_TAIL_SUPPRESSION_MS = 700;
...
let suppressInputTailUntil = 0;
let suppressInputTailTarget = '';
...
function initNotificationWindowOwner(){
  ...
  // A real gesture can arrive as PointerEvent, TouchEvent or only a generated
  // click depending on Telegram WebView and platform. All three paths converge
  // on one owner. The first valid end opens the sheet; compatibility tails from
  // the same gesture are consumed without blocking a new physical press.
...
function handleTouchEnd(event){
  const touchState = activeTouch;
  activeTouch = null;
  if (!touchState) return;

  const touch = Array.from(event.changedTouches || [])
    .find(item => item.identifier === touchState.identifier) || event.changedTouches?.[0] || null;
  if (!touch) return;

  const trigger = notificationTrigger(event.target);
  if (!trigger || trigger.id !== touchState.trigger) return;
  ...
  event.stopImmediatePropagation();
  claimGesture(touchState.trigger);
}
...
function handleClickFallback(event){
  ...
  if (isCompatibilityTail(trigger.id)) {
    suppressInputTailUntil = 0;
    suppressInputTailTarget = '';
    return;
  }

  claimGesture(trigger.id);
}
...
function beginNewPhysicalInput(){
  suppressInputTailUntil = 0;
  suppressInputTailTarget = '';
}

function claimGesture(triggerId){
  if (isCompatibilityTail(triggerId)) return false;
  suppressInputTailUntil = Date.now() + INPUT_TAIL_SUPPRESSION_MS;
  suppressInputTailTarget = triggerId;
  openFromUserInput();
  return true;
}

function isCompatibilityTail(triggerId){
  return Date.now() <= suppressInputTailUntil && triggerId === suppressInputTailTarget;
}
