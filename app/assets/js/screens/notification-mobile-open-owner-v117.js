...
initMobileNotificationOpenOwner();

function initMobileNotificationOpenOwner(){
...
async function openNotificationsSheet({ hapticFeedback = true, keepVisible = false } = {}){
  ...
  if (freshItems().length) {
    renderNotifications(freshItems());
  } else if (!keepVisible || !isNotificationsSheetOpen()) {
    renderLoading();
  }
  ...
  try {
    ...
    if (!items.length) {
      ...
      const confirmed = await api.notifications(true);
      const confirmedItems = normalizeItems(confirmed?.items);
      if (confirmedItems.length || !result?.ok) result = confirmed;
...
function isMobileSurface(){
  return window.matchMedia('(max-width: 760px)').matches || (navigator.maxTouchPoints > 0 && window.innerWidth < 900);
}
...
function notificationMessage(item){
  return String(item?.message || '').replace(/\s+/g, ' ').trim();
}
