...
export function initNotificationsScreen(){
  ...
  // This request is a baseline snapshot, but it must not swallow a notification
  // that is created while the request itself is in flight.
  refreshNotificationBadge(false);
...
export async function refreshNotificationBadge(announce = false){
  ...
  try {
    ...
    if (!baselineLoaded || !announce || !appReady) {
      ...
      baselineLoaded = true;
      const candidate = freshDuringRequest.find(item => !item?.read);
      if (candidate) {
...
function rememberBaselineNotifications(items, requestStartedAt){
  ...
  for (const item of items) {
    ...
    if (!item?.read && createdAt > threshold) {
      fresh.push(item);
      continue;
    }
    announcedIds.add(id);
...
function announceNotification(item){
  const id = String(item?.id || '');
  if (!id || announcedIds.has(id)) return false;
...
function ensureNotificationToast(){
  ...
  // Opening is intentionally absent here. notification-window-owner-v124 owns
  // pointer, touch, click and keyboard activation for the bell and this toast.
