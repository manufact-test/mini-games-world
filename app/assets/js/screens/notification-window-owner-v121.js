...
function initNotificationWindowOwner(){
  ...
  // One canonical owner handles the complete real pointer sequence. Opening on
  // pointerup avoids relying on a browser-generated click that Telegram WebView
  // may suppress after a very short press. The following click is consumed only
  // as the tail of that same gesture, never used as a retry.
...
function handleClickFallback(event){
  ...
  // Keyboard activation, accessibility tools and browsers without a complete
  // PointerEvent sequence still receive one deterministic fallback action.
  openFromUserInput();
}
