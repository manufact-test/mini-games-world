...
import { buildTicTacToeOptimistic, gameSurfaceFingerprint } from '../production-v97-models.js?v=97';
import { buildBattleshipSetupOptimistic, pollResultIsCurrent } from '../production-v99-models.js?v=99';

const runtime = window.__MGW_V99_GAME_RUNTIME__ ||= {
  initialized:false,
  games:new Map(),
  pointerHoldUntil:0,
...
export function initGameScreen(){
  ...
  document.addEventListener('pointerdown', event => {
    ...
    if (origin instanceof Element && origin.closest('#gameBoard button')) {
  ...
  const release = event => {
    ...
    if (origin instanceof Element && origin.closest('#gameBoard')) {
      runtime.pointerHoldUntil = Date.now() + 120;
    }
  };
...
export function clearGameView(){
  ...
  if (board) {
    ...
    delete board.dataset.mgwV99Fingerprint;
  }
...
async function refreshGame(gameId){
  ...
  if (item.pollBusy || item.running || item.queue.length) return;
  ...
  try {
    ...
    if (!pollResultIsCurrent(generation, item.generation, item.running || item.queue.length)) return;
...
function submitAction(gameId, action){
  ...
  if (!base || String(base.id || '') !== id || String(base.status || '') !== 'active') return;
  ...
  haptic('light');
  const optimistic = buildOptimisticSnapshot(base, action, viewer.id, gameTypeOf(base));
  if (optimistic) {
...
async function drainActions(gameId, item){
  ...
  try {
    while (item.queue.length) {
      ...
      if (String(authoritative.status || '') === 'finished') {
        finishGame(authoritative, item.viewer);
      }
    }
  } finally {
    item.running = false;
...
function buildOptimisticSnapshot(game, action, viewerId, type){
  if (type === 'tictactoe') return buildTicTacToeOptimistic(game, action, viewerId);
  if (type === 'battleship' && String(game?.phase || '') === 'setup') {
    return buildBattleshipSetupOptimistic(game, action);
  }
  return buildOptimisticGame(game, action, viewerId, type);
}
...
function renderGame(game, me, forceSurface){
  ...
  const rendered = String(surface.dataset.mgwV99Fingerprint || '');
  if (forceSurface || surface.childElementCount === 0 || fingerprint !== rendered) {
    ...
    surface.dataset.mgwV99Fingerprint = fingerprint;
  }
}
...
async function confirmLeaveGame(){
  ...
  try {
    ...
    closeSheet();
    if (result?.game) {
      const item = gameRuntime(String(result.game.id));
      const viewer = normalizeViewer(result.me) || item.viewer || resolveViewer(result.game);
      if (viewer) {
        item.viewer = viewer;
        item.authoritative = clone(result.game);
        item.optimistic = clone(result.game);
        finishGame(result.game, viewer);
        return;
      }
    }
    state.activeGame = null;
