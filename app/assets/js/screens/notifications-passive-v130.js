...
function ensureNotificationToast(){
  ...
  el.innerHTML = `<div class="notification-toast-icon" aria-hidden="true">🔔</div><div class="notification-toast-copy"><strong></strong><span></span></div>`;
...
function showNotificationToast(item){
  ...
  const tone = ['success', 'danger', 'warning', 'info'].includes(String(item?.tone || '')) ? String(item.tone) : 'info';
...
function canShowNotificationToast(){
  if (window.__MGW_INVITE_LINK_OPENING__ === true) return false;
