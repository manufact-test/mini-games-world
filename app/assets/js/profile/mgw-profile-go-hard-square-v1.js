...
export function initProfileGoHardSquare(){
  ...
  screen.addEventListener('click', event => {
    ...
    if (!target) return;
    if (target.closest('[data-profile-game-tab="go"], [data-profile-game-cosmetic^="game-go-"]')) {
      scheduleHardSquareRepair(screen);
    }
  });
...
function scheduleHardSquareRepair(screen){
  const run = () => enforceProfileGoSquares(screen);
  queueMicrotask(run);
...
function enforceProfileGoSquares(screen){
  if (!(screen instanceof HTMLElement)) return;
  const goTab = screen.querySelector('[data-profile-game-tab="go"]');
  const active = goTab instanceof HTMLElement
    && (goTab.classList.contains('active') || goTab.getAttribute('aria-selected') === 'true');
  if (!active) return;

  screen.querySelectorAll('.profile-v2-game-card .store-v2-game-preview[data-game-type="go"]').forEach(preview => {
    if (!(preview instanceof HTMLElement)) return;
    const width = preview.getBoundingClientRect().width;
    if (!(width > 0)) return;
    const px = `${Math.round(width * 100) / 100}px`;

    preview.dataset.mgwProfileGoHardSquare = '1';
    ...
    const artwork = preview.querySelector(':scope > .mgw-go-preview');
    ...
    const board = artwork.querySelector(':scope > .mgw-go-board');
