...
const runtime = window.__MGW_V101_SHARE__ ||= {
  initialized:false,
  sequence:0,
...
export function initV101ShareController(){
...
async function startShare(button){
  if (runtime.active?.opening) return;
  ...
  const attempt = { id:++runtime.sequence, opening:true, button, context, key, draft:null, timeout:null };
...
function openPreparedMessage(telegram, preparedId, attempt){
  ...
  const finish = sent => {
    ...
    if (sent === false) discardDraft(attempt.draft);
  };
