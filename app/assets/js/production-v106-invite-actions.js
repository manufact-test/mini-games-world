...
const runtime = window.__MGW_V106_INVITE_ACTIONS__ ||= {
  ...
  syncBusy:false,
  selfToastUntil:0,
  selfToastMessages:new Set(),
};

const SELF_CONFIRMATIONS = new Set([
...
export function initV106InviteActions(){
  if (runtime.initialized) return;
  runtime.initialized = true;

  window.addEventListener('click', ownInviteAction, true);
  installSelfToastFilter();
  ...
  if (typeof tg?.onEvent === 'function') {
    try {
      tg.onEvent('activated', () => window.setTimeout(() => refreshActivatedInvite(true), 0));
    } catch (error) {
      // Older clients use visibilitychange as the fallback.
    }
  }
}
...
async function performImmediateAction(action, token, requestKey){
  haptic('light');
  abortCompetingReads(`v106-invite-${action}`);
  ...
  if (action === 'accept') {
    renderAcceptedImmediately(token, summary);
  } else if (action === 'start') {
    showPendingGameLaunch(summary);
  } else {
    suppressSelfConfirmation(action);
    closeSheet();
  ...
  try {
    ...
    if (action === 'accept' && invite?.token) {
      renderInviteeWaiting(invite);
    } else if (action === 'start') {
      throw new Error('Матч не запустился. Попробуйте ещё раз.');
    }
...
function renderAcceptedImmediately(token, summary){
  openSheet(`
    <span data-invite-sheet data-invite-token="${escapeHtml(token)}" data-invite-state="accepted:invitee" hidden></span>
...
function renderInviteeWaiting(invite){
  openSheet(`
    <span data-invite-sheet data-invite-token="${escapeHtml(invite.token || '')}" data-invite-state="accepted:invitee" hidden></span>
    <div class="sheet-head">
...
async function refreshActivatedInvite(fromTelegramActivation){
  if (!runtime.appReady || runtime.syncBusy || document.visibilityState !== 'visible') return;
  if (String(state.activeGame?.status || '') === 'active') return;
  ...
  try {
    ...
    if (launchToken && launchToken !== runtime.lastLaunchToken) {
      ...
      result = await inviteRequest('sync', {});
    }
    ...
    } else if (fromTelegramActivation && !overlayActive && homeActive && String(invite.status || '') === 'accepted' && invite.is_owner) {
      renderOwnerReady(invite);
    }
  } catch (error) {
    // The retained sync loop remains the fallback; activation refresh is best-effort.
  } finally {
...
function renderOwnerReady(invite){
  openSheet(`
    <span data-invite-sheet data-invite-token="${escapeHtml(invite.token || '')}" data-invite-state="accepted:owner" hidden></span>
    <div class="sheet-head">
...
function installSelfToastFilter(){
  const target = document.getElementById('toast');
  if (!target || typeof MutationObserver !== 'function') return;
  const observer = new MutationObserver(() => {
    if (Date.now() > runtime.selfToastUntil) return;
    const message = String(target.textContent || '').trim();
    if (!runtime.selfToastMessages.has(message) && !SELF_CONFIRMATIONS.has(message)) return;
    target.classList.remove('show');
    target.textContent = '';
  });
  observer.observe(target, {childList:true, characterData:true, subtree:true, attributes:true, attributeFilter:['class']});
}

function suppressSelfConfirmation(action){
  runtime.selfToastUntil = Date.now() + 5000;
  runtime.selfToastMessages = new Set(SELF_CONFIRMATIONS);
  if (action === 'cancel') runtime.selfToastMessages.add('Приглашение отменено.');
  if (action === 'decline') runtime.selfToastMessages.add('Приглашение отклонено.');
}
