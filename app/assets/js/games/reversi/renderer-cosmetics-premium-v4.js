...
function ensureTailSettleStyles(){
  if (typeof document === 'undefined') return;
  const href = new URL('../../../css/games/reversi/live-effects-tail-settle-v6.css?v=1&mvp19_7=tail-finish-early-v1', import.meta.url).href;
  const existing = document.querySelector('link[data-mgw-reversi-live-effects-tail-settle]');
  if (existing instanceof HTMLLinkElement) {
    if (existing.href !== href) existing.href = href;
    document.head.appendChild(existing);
    return;
  ...
  const link = document.createElement('link');
  link.rel = 'stylesheet';
  link.dataset.mgwReversiLiveEffectsTailSettle = 'mvp19-7-tail-v6';
  link.href = href;
  document.head.appendChild(link);
}
