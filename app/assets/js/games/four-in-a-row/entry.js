...
import { startSearchPolling } from '../../screens/search-screen.js?v=52';
import { startGamePolling } from '../../screens/game-screen.js?v=52';
...
function openFourInARowSetup(){
  ...
  const sizeChoices = Object.entries(FOUR_VARIANTS).map(([size, variant]) => `
    <button class="choice ${Number(size) === selectedSize ? 'active' : ''}" data-four-board-size="${size}" type="button">
  ...
  openSheet(`
    <div class="sheet-head">
      <div><h2>4 в ряд</h2><p>${roomName(state.room)}. Выберите размер поля${isGold ? ' и стоимость участия' : ''}.</p></div>
      <button class="close" data-close-sheet type="button">×</button>
    </div>

    <div class="setup-scroll four-setup-scroll">
      <div class="small-note">Выбирайте столбец — фишка сама упадёт вниз. Побеждает тот, кто первым соберёт четыре фишки подряд.</div>

      <div class="section-title"><h2>Поле</h2></div>
      <div class="choice-grid four-size-grid" id="fourSizeChoices">${sizeChoices}</div>

      <div id="fourSetupPreview">${fourPreviewMarkup(selectedSize)}</div>

      <div class="section-title"><h2>Стоимость участия</h2></div>
      <div class="choice-grid ${isGold ? '' : 'single-choice'}" id="fourBetChoices">${betChoices}</div>
    </div>
  ...
  document.querySelectorAll('[data-four-board-size]').forEach(button => button.addEventListener('click', () => {
    ...
    document.querySelectorAll('[data-four-board-size]').forEach(item => {
...
async function startFourInARowSearch(){
  ...
  try {
    ...
    if (info) {
      info.textContent = `4 в ряд · ${roomName(state.room)} · участие ${bet} коинов · поле ${variant.meta}`;
    }
...
function normalizeFourSize(value){
  return Object.prototype.hasOwnProperty.call(FOUR_VARIANTS, Number(value))
    ? Number(value)
    : 7;
}
...
function fourPreviewMarkup(size){
  ...
  const place = (row, col, value) => {
    if (row < 0 || row >= variant.rows || col < 0 || col >= variant.columns) return;
    cells[row * variant.columns + col] = value;
  };
  ...
  return `
    <div
      class="four-setup-preview"
      style="--four-columns:${variant.columns};--four-rows:${variant.rows}"
      aria-hidden="true"
    >
      ${cells.map(value => `<span class="${value}"></span>`).join('')}
    </div>
    <div class="four-preview-caption">${variant.label} поле · ${variant.meta}</div>
  `;
}
