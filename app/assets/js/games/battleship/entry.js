...
export function initBattleshipEntry(){
  ...
  document.addEventListener('click', event => { if (event.target.closest('#playBattleship')) openBattleshipSetup(); });
}

function openBattleshipSetup(){
  if (isSessionLocked(state.session)) return toast(sessionMessage(state.session));
  haptic('light'); state.selectedGame = 'battleship';
  const isGold = state.room === 'gold';
  const selectedBet = state.room === 'match' ? APP_CONFIG.matchBet : Number(state.selectedBet || APP_CONFIG.goldBets[0]);
  ...
  openSheet(`
    <div class="sheet-head"><div><h2>Морской бой</h2><p>${roomName(state.room)}. Классическое поле 10×10.</p></div><button class="close" data-close-sheet type="button">×</button></div>
    <div class="setup-scroll battleship-start-scroll"><div class="small-note">После подбора соперника у вас будет 2 минуты на расстановку. Флот можно перемешивать без ограничений или расставить вручную.</div><div class="battleship-start-preview" aria-hidden="true">${previewCells()}</div><div class="battleship-preview-caption">Поле 10×10 · 10 кораблей</div><div class="battleship-start-fleet">${fleetPreviewItem(4,1)}${fleetPreviewItem(3,2)}${fleetPreviewItem(2,3)}${fleetPreviewItem(1,4)}</div><div class="section-title"><h2>Стоимость участия</h2></div><div class="choice-grid ${isGold ? '' : 'single-choice'}" id="battleshipBetChoices">${betChoices}</div></div>
...
async function startBattleshipSearch(){
  ...
  try {
    ...
    state.user = result.user || state.user; state.selectedBet = bet; renderBalances(state.user);
    if (result.game) { state.activeGame = result.game; state.selectedGame = 'battleship'; showScreen('game'); startGamePolling(result.game.id); return; }
    const info = document.getElementById('searchInfo');
    if (info) info.textContent = `Морской бой · ${roomName(state.room)} · участие ${bet} коинов · поле 10×10`;
    showScreen('search'); startSearchPolling();
  } catch (error) { toast(error.message); }
}

function previewCells(){
  const ships = new Set([2,12,22,32,56,57,58,71,72,89]);
  return Array.from({length:100},(_,cell)=>`<span class="${ships.has(cell) ? 'ship' : ''}"></span>`).join('');
}
function fleetPreviewItem(size,count){ return `<div><span class="ship-shape">${'<i></i>'.repeat(size)}</span><strong>×${count}</strong></div>`; }
