...
const runtime = window.__MGW_V109_SHARE_SPEED__ ||= {
  initialized:false,
  sequence:0,
...
export function initV109ShareSpeed(){
  if (runtime.initialized) return;
  runtime.initialized = true;

  window.addEventListener('pointerdown', rememberAndWarm, true);
  window.addEventListener('click', ownShareClick, true);

  document.addEventListener('mgw:v109-invite-slot-free', event => {
    const context = normalizeContext(event.detail?.context || window.__MGW_V109_LAST_INVITE_CONTEXT__ || defaultContext(runtime.lastGameType));
    window.setTimeout(() => void warmContextSafely(context), 40);
  });

  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') releaseReturnedNativeAttemptSoon();
  });

  const telegram = getTelegram();
  if (typeof telegram?.onEvent === 'function') {
    try { telegram.onEvent('activated', releaseReturnedNativeAttemptSoon); } catch (error) {}
  }
}

function rememberAndWarm(event){
...
async function startShare(button){
  ...
  try {
    ...
    showPreparedLink(draft, context);
    runtime.active = null;
  } catch (error) {
...
function openPreparedMessage(telegram, preparedId, attempt){
  ...
  const finish = sent => {
    ...
    window.clearTimeout(runtime.returnTimer);
    ...
    if (sent === false) {
      discardDraft(attempt.draft).finally(() => {
        document.dispatchEvent(new CustomEvent('mgw:v109-invite-slot-free', {
          detail:{ context:attempt.context },
        }));
      });
    }
  };
  ...
  try {
    ...
    attempt.nativePending = false;
    restoreShareSurface(attempt.surface);
    showPreparedLink(attempt.draft, attempt.context);
    runtime.active = null;
  }
}

function releaseReturnedNativeAttemptSoon(){
  const attempt = runtime.active;
  if (!attempt?.nativePending) return;
  attempt.returned = true;
  window.clearTimeout(runtime.returnTimer);
  runtime.returnTimer = window.setTimeout(() => {
    const current = runtime.active;
    if (!current?.nativePending || current.id !== attempt.id) return;

    // Some Telegram clients return to the Mini App without resolving the native
    // callback. Release the local lock quickly and leave the draft valid; if the
    // message was sent, opening its link still activates it authoritatively.
    window.clearTimeout(current.timeout);
    current.nativePending = false;
    restoreShareSurface(current.surface);
    runtime.active = null;
  }, RETURN_RELEASE_MS);
}
...
function showPreparedLink(invite, context){
  ...
  openSheet(`
    <span data-invite-sheet data-invite-token="${escapeHtml(invite?.token || '')}" data-invite-state="draft:owner" hidden></span>
    ...
    <div class="stack invite-actions">
      <button class="btn primary full" data-v109-fallback-share type="button">Открыть Telegram</button>
      <button class="btn ghost full" data-v109-copy-link type="button">Скопировать ссылку</button>
      <button class="btn ghost full" data-v109-discard-draft type="button">Отменить</button>
    </div>
  `);

  document.querySelector('[data-v109-fallback-share]')?.addEventListener('click', () => openFallbackShare(invite));
  document.querySelector('[data-v109-copy-link]')?.addEventListener('click', () => copyInviteLink(shareUrl));
  document.querySelector('[data-v109-discard-draft]')?.addEventListener('click', async () => {
    await discardDraft(invite);
    restoreShareSurface(captureShareSurface());
    document.dispatchEvent(new CustomEvent('mgw:v109-invite-slot-free', { detail:{ context } }));
  });
}
...
function inviteSummary(invite){
  ...
  const columns = Number(invite?.board_columns || invite?.board_size || 0);
  const rows = Number(invite?.board_rows || invite?.board_size || columns);
  const variant = String(invite?.game_type || '') === 'domino'
    ? 'Классика 0–6'
    : (columns > 0 ? `${columns}×${rows}` : 'Стандартный');
  return `
...
function openSheetInviteToken(){
  return String(document.querySelector('[data-invite-sheet]')?.dataset.inviteToken || '');
}
