...
export function buildV100OptimisticGame(game, action, viewerId, gameType){
  ...
  if (type === 'tictactoe') {
    return buildTicTacToeOptimistic(game, action, id);
  }
  ...
  optimistic.__mgw_v100_pending_action = normalizePendingAction(action);
  return optimistic;
}

export function pendingSurfaceDescriptor(game, gameType){
  const type = String(gameType || '');
  const action = game?.__mgw_v100_pending_action || null;

  if (type === 'battleship') {
    const cell = Number(game?.pending_fire_cell ?? action?.cell);
    if (Number.isInteger(cell) && cell >= 0 && cell < 100) {
      return {
        selector:`[data-battleship-cell="${cell}"]`,
        className:'mgw-pending-shot',
      };
    }
  }
  ...
  if (type === 'four_in_a_row') {
    ...
    if (Number.isInteger(column)) {
      return {
        selector:`[data-four-column="${column}"]`,
        className:'mgw-pending-action',
      };
    }
  }
...
export function invalidateInFlightPoll(runtime, gameId){
