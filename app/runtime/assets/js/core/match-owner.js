...
export function createMatchOwner({ root, api, store, router, requestContext }){
  ...
  function render(state){
    if (state.matchResult) {
      renderResult(state.matchResult, state.balances);
      router.show('result');
      stopPolling();
      return;
    }
    if (state.activeMatch) {
      renderMatch(state.activeMatch, state.balances);
      router.show('match');
      startPolling();
      return;
    }
    if (state.matchmaking) {
      renderSearch(state.matchmaking, state.balances);
      router.show('search');
      startPolling();
      return;
    }
  ...
  function renderSearch(matchmaking, balances){
    setText('[data-search-bet]', String(matchmaking.bet ?? 10));
    setText('[data-search-balance]', String(balances?.match ?? '—'));
  }

  function renderMatch(match, balances){
    ...
    setText('[data-match-status]', match.turn === match.viewer_id ? 'Ваш ход' : 'Ход соперника');
    setText('[data-match-symbol]', viewer?.symbol || match.viewer_symbol || '—');
    setText('[data-game-balance]', String(balances?.match ?? '—'));
  ...
  function renderResult(result, balances){
    ...
    setText('[data-result-balance]', String(balances?.match ?? '—'));
    setText('[data-result-payout]', String(result.payout ?? 0));
  }
