...
$assertTrue(str_contains($activeStore, 'store-screen-checkers-board-source-wrapper.js?v=7') && str_contains($activeStore, 'domino_preview=native-v10') && str_contains($activeStore, 'domino_effects=premium-v11') && str_contains($activeStore, 'selector_center=instant-v1') && str_contains($activeStore, 'domino_base=native-render-v1'), 'Active Store import must preserve the accepted owner and publish premium Domino v11 plus stable selector centering');
...
$assertTrue(str_contains($storeOwner, "store-screen-reversi-store-v1.js?v=1&mvp19_7=store-only&review=manual-corrective-v2") && str_contains($storeOwner, "store-screen-domino-store-v1.js?v=6&mvp19_9=domino-native-render-v6") && str_contains($storeOwner, "store-screen-domino-effects-v9.js?v=3&mvp19_9=domino-premium-effects-v11") && str_contains($storeOwner, 'installDominoStorePresentation();') && str_contains($storeOwner, 'upgradeDominoStorePresentation();'), 'Accepted Store owner must retain its Reversi import identity, native Domino and premium effect graph');
...
$assertTrue(str_contains($profileModule, 'store-card-fill-live-pips-v5.css?v=2&mvp19_9=domino-card-fill-live-pips-v6') && str_contains($profileModule, 'store-effects-scene-v9.css?v=3&mvp19_9=domino-premium-effects-v11'), 'Profile must load the accepted static pip style and Store premium v11 effect style');
...
$assertTrue(str_contains($effectCss, '@media(prefers-reduced-motion:reduce)') && str_contains($effectCss, 'aspect-ratio:47 / 24!important') && str_contains($effectCss, 'mgw-domino-v11-magnetic-glide') && str_contains($effectCss, 'mgw-domino-v11-spectral-draw') && str_contains($effectCss, 'mgw-domino-v11-lux-sheen'), 'Domino v11 effects must retain fixed whole-piece geometry, premium continuous choreography and reduced-motion fallback');
$assertTrue(!str_contains($effectCss, 'mgw-domino-v10-precision-glide') && !str_contains($effectCss, 'mgw-domino-v10-stock-flight') && !str_contains($effectCss, 'mgw-domino-v10-finale-wave'), 'Rejected v10 staged Domino choreography must be removed completely');
...
$launchMatch = [];
$assertTrue(preg_match('~/app/v110\.php\?v=(\d+)~', $launch, $launchMatch) === 1 && (int)$launchMatch[1] >= 1166, 'Telegram launch must publish the premium Domino v11 graph');

echo "MVP-19.9 Domino Store/Profile premium v11 contract passed ({$assertions} assertions).\n";