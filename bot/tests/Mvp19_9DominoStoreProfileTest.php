...
$assertTrue(str_contains($storeModule, 'native:v12:motion-v46') && str_contains($storeModule, 'data-mgw-domino-preview-component="v44"') && str_contains($storeModule, 'sceneReady'), 'Store must publish fresh v44 preview identity and reject stale scene markup');
...
$assertTrue(str_contains($baseStore, "store-screen-domino-store-v1.js?v=12&mvp19_9=domino-preview-motion-v46"), 'Base Store must directly import animated v46 instead of relying on wrapper repair');
...
$assertTrue(str_contains($storeOwner, "store-screen-domino-store-v1.js?v=12&mvp19_9=domino-preview-motion-v46") && str_contains($storeOwner, "store-screen-domino-effects-v9.js?v=10&mvp19_9=domino-preview-motion-v46"), 'Store owner must wire fresh v46 preview motion over accepted base geometry');
...
$assertTrue(str_contains($profileModule, "dominoPreviewMarkup } from '../screens/store-screen-domino-store-v1.js?v=12&mvp19_9=domino-preview-motion-v46'") && str_contains($profileModule, 'store-effects-scene-v9.css?v=7&mvp19_9=domino-premium-effects-v15-proportions') && str_contains($profileModule, 'store-effects-preview-component-v44.css?v=2&mvp19_9=domino-preview-motion-v46'), 'Profile must reuse the exact Store v46 preview source and component CSS');
...
$assertTrue(str_contains($activeStore, 'domino_effects=motion-v46') && str_contains($activeStore, 'domino_preview=motion-v46') && str_contains($activeStore, 'domino_base=native-render-v2'), 'Active Store graph must publish v46 previews without changing the accepted base owner');
...
$assertTrue(str_contains($activeProfile, 'domino_effects=motion-v46') && str_contains($activeProfile, 'domino_preview=shared-motion-v46'), 'Active Profile graph must publish v46 parity');
$assertTrue(str_contains($dominoSource, 'store-screen-domino-store-v1.js?v=12') && str_contains($dominoCachedSource, 'store-screen-domino-store-v1.js?v=12') && str_contains($dominoProfile, 'mgw-profile-domino-parity.js?v=12'), 'Import map must cache-bust Store, cached base import and Profile to v46 sources');
$launchMatch = [];
...
echo "MVP-19.9 Domino Store/Purchase/Profile v46 animated preview contract passed ({$assertions} assertions).\n";
