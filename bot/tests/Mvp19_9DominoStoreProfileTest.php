...
$assertTrue(str_contains($previewComponentCss, 'Animated properties intentionally do NOT use !important'), 'Preview CSS must document keyframe ownership');
$assertTrue(str_contains($previewComponentCss, 'Preview-only pip geometry scales with tile size') && str_contains($previewComponentCss, 'width:clamp(1.25px,32%,3px)!important') && str_contains($previewComponentCss, 'content:none!important'), 'Effect preview pips must scale with tile size and suppress the legacy fixed 3px pseudo-pip');
$assertTrue(str_contains($previewComponentCss, 'Stock beam geometry is pinned to the exact source/target centers') && str_contains($previewComponentCss, 'width:60.2%!important') && str_contains($previewComponentCss, 'rotate(20deg)'), 'Stock beam/orb must share one exact source-to-target trajectory');
$assertTrue(str_contains($storeModule, 'function effectTileMarkup') && str_contains($storeModule, 'data-mgw-domino-face="svg-v1"') && str_contains($storeModule, 'mgw-domino-v48-face-svg') && str_contains($storeModule, 'mgw-domino-v48-pip'), 'All effect tiles must use one canonical SVG pip renderer');
$assertTrue(str_contains($previewComponentCss, 'v48 canonical effect-tile face') && str_contains($previewComponentCss, '.mgw-domino-v48-pip') && str_contains($previewComponentCss, 'fill:#171a20!important'), 'Canonical SVG pips must keep one round dark visual across all effects');
...
$assertTrue(str_contains($storeOwner, "store-screen-domino-store-v1.js?v=15&mvp19_9=domino-svg-pips-v48&paid_default=copy-native-v1") && str_contains($storeOwner, "store-screen-domino-effects-v9.js?v=12&mvp19_9=domino-svg-pips-v48"), 'Store owner must wire fresh v48 preview motion plus canonical replacement copy over accepted base geometry');
...
$assertTrue(str_contains($baseStoreTarget, 'store-screen.js?v=53'), 'Base Store target must publish the canonical no-flicker copy revision over the accepted native base');
...
$assertTrue(str_contains($dominoSource, 'store-screen-domino-store-v1.js?v=14') && str_contains($dominoCachedSource, 'store-screen-domino-store-v1.js?v=14') && str_contains($dominoProfile, 'mgw-profile-domino-parity.js?v=14'), 'Import map must cache-bust Store, cached base import and Profile to v48 sources');
$launchMatch = [];
...
echo "MVP-19.9 Domino Store/Purchase/Profile v48 SVG-pip preview contract passed ({$assertions} assertions).\n";
