...
$assertTrue(str_contains($storeModule, 'native:v12:component-v44-fix-v45') && str_contains($storeModule, 'data-mgw-domino-preview-component="v44"') && str_contains($storeModule, 'sceneReady'), 'Store must publish fresh v44 preview identity and reject stale scene markup');
...
$assertTrue(str_contains($previewComponentCss, 'Animated properties intentionally stay non-!important'), 'v45 must document the animation cascade invariant');
$assertTrue(!str_contains($previewComponentCss, 'left:var(--sx)!important') && !str_contains($previewComponentCss, 'top:var(--sy)!important'), 'Particle positions must remain keyframe-controlled');
$assertTrue(!str_contains($previewComponentCss, 'transform:translate(-50%,-50%) scale(.82)!important') && !str_contains($previewComponentCss, 'transform:rotate(27deg) scaleX(0)!important') && !str_contains($previewComponentCss, 'transform:translateX(-44%) skewX(-4deg)!important'), 'Animated transforms must not be blocked by important declarations');
$assertTrue(!str_contains($profileCss, 'animation:none!important'), 'Profile CSS must not freeze Domino effect previews');
$assertTrue(str_contains($baseStore, "store-screen-domino-store-v1.js?v=12&mvp19_9=domino-preview-component-v44-fix-v45"), 'Base Store must directly import v44 instead of relying on wrapper repair');
...
$assertTrue(str_contains($storeOwner, "store-screen-domino-store-v1.js?v=12&mvp19_9=domino-preview-component-v44-fix-v45") && str_contains($storeOwner, "store-screen-domino-effects-v9.js?v=10&mvp19_9=domino-preview-component-v44-fix-v45"), 'Store owner must wire fresh v44 preview markup over accepted base geometry');
...
$assertTrue(str_contains($profileModule, "dominoPreviewMarkup } from '../screens/store-screen-domino-store-v1.js?v=12&mvp19_9=domino-preview-component-v44-fix-v45'") && str_contains($profileModule, 'store-effects-scene-v9.css?v=7&mvp19_9=domino-premium-effects-v15-proportions') && str_contains($profileModule, 'store-effects-preview-component-v44.css?v=2&mvp19_9=domino-preview-component-v44-fix-v45'), 'Profile must reuse the exact Store v44 preview source and component CSS');
...
$assertTrue(str_contains($activeStore, 'domino_effects=component-v44-fix-v45') && str_contains($activeStore, 'domino_preview=component-v44-fix-v45') && str_contains($activeStore, 'domino_base=native-render-v2'), 'Active Store graph must publish v44 previews without changing the accepted base owner');
...
$assertTrue(str_contains($activeProfile, 'domino_effects=component-v44-fix-v45') && str_contains($activeProfile, 'domino_preview=shared-component-v44-fix-v45'), 'Active Profile graph must publish v44 parity');
