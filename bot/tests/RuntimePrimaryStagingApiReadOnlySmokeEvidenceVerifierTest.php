...
try {
    ...
    $changed = $base;
    $changed['state_revision'] = '3';
    $write($changed);
    $assertThrows(static fn() => $verify(), 'state revision must be a positive integer');
    ...
    $changed = $base;
    $changed['top_level_count'] = 257;
    $write($changed);
    $assertThrows(static fn() => $verify(), 'top-level state count is outside safe bounds');

    $changed = $base;
    $changed['ttl_seconds'] = '300';
    $write($changed);
    $assertThrows(static fn() => $verify(), 'ttl must be a positive integer');

    $changed = $base;
    $changed['bootstrap_legacy_hook_count'] = '5';
    $write($changed);
    $assertThrows(static fn() => $verify(), 'hook/filter contour');
    ...
    $changed = $base;
    $changed['generated_at_utc'] = '2027-02-30T00:00:00+00:00';
    $write($changed);
    $assertThrows(static fn() => $verify(), 'timestamp is invalid');
