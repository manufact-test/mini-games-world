...
foreach ([34, 38, 46, 47] as $version) {
    ...
    $assert(str_contains($url, 'mvp16=profile-corrective&mvp17=history-fresh-match&menu=fast-history'), "API alias v{$version} must preserve the accepted fast menu History prefix across the name-color cache bump.");
}
$assert(
...
$assert(is_string($launch) && str_contains($launch, "private const ENTRY_PATH = '/app/v110.php?v=1233&"), 'Corrective pass must remain anchored to the actual Telegram v110 entry.');
