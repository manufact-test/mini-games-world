...
$assert(is_string($home) && str_contains($home, "const game=item.game_title||'Матч';"), 'Actual History modal must display the real game title.');
$assert(is_string($home) && str_contains($home, "const economy=item.economy&&typeof item.economy==='object'?item.economy:null;"), 'Actual History modal must consume the canonical viewer economy projection.');
$assert(is_string($home) && str_contains($home, 'economy.ledger_delta'), 'Actual History modal must display the viewer ledger delta.');
$assert(is_string($home) && str_contains($home, 'Вход:') && str_contains($home, 'Награда:') && str_contains($home, 'Итог:') && str_contains($home, 'Баланс:'), 'Actual History modal must label all four economy values explicitly.');
$assert(is_string($home) && !str_contains($home, 'const payout=item.payout'), 'Actual History modal must never use the global winner payout as the viewer result.');
$assert(is_string($home) && !str_contains($home, '#${escapeHtml(item.short_id'), 'Actual History modal must not expose technical match hashes.');
$assert(is_string($home) && !str_contains($home, '· ставка ${bet} коинов'), 'Actual History modal must not use the old raw stake-only presentation.');
