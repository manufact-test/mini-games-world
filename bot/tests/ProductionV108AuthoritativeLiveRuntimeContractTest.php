...
$assert(
    ...
    'The isolated v108 source must remain reproducible while inactive.'
);
...
$assert(
    ...
    'The rejected client sources must remain available for root-cause analysis.'
);

$assert(
    str_contains($registry, "'bot/game-live-v108.php' => 'game_live_v108'"),
    'The registered endpoint remains guarded even while the v108 client is disabled.'
);

$assert(
    str_contains($php, 'production-clean-entry-v105.js?v=105')
        && str_contains($php, 'main-v105.js?v=105')
        && str_contains($php, 'v105-mvp14-emergency-rollback')
    ...
    'Every existing v108 URL must serve the accepted v105 graph after emergency rollback.'
);
