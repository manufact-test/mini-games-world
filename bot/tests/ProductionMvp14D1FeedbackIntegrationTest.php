...
$assert(str_contains($opponents, 'REQUIRED_AUTHORITATIVE_EMPTY_RESPONSES = 2')
    ...
    && !str_contains($presence, 'openSheet('), 'v122 must confirm DB-primary empties without UI or presence ownership.');
$assert(str_contains($main, 'state.stats = mergePresenceOnline(result.stats);')
    ...
    && str_contains($presence, 'window.__MGW_V115_PRESENCE_ONLINE__'), 'Presence-owned online count must survive bootstrap and polling.');
