...
$assert(str_contains($indexHtml, './assets/js/entry.js?v=7'), 'Clean runtime document must load the surrender-transition entry module.');
$assert(substr_count($entry, 'import ') === 1 && str_contains($entry, './core/app.js?v=7'), 'Clean entry must cache-bust the surrender-transition application bootstrap.');
...
$assert(str_contains($app, "./store.js?v=7") && str_contains($app, "./match-owner.js?v=7"), 'Modified surrender-transition modules must be cache-busted independently.');
$assert(str_contains($app, "CLIENT_BUILD = 'clean-client-v7-surrender-transition'"), 'Clean client must expose an exact surrender-transition build marker.');
...
$assert(str_contains($store, 'matchTransition:null'), 'The clean store must expose one explicit match transition slice.');
$assert(str_contains($matchOwner, "type:'surrendering'") && str_contains($matchOwner, "router.show('home')"), 'Manual surrender must enter an explicit transition and route home immediately.');
$assert(str_contains($matchOwner, "transition.next !== 'start-search'") && str_contains($matchOwner, "next:'start-search'"), 'A play request during surrender must be queued in the same match owner.');
$assert(str_contains($matchOwner, 'await api.dismissResult(context') && str_contains($matchOwner, 'finalProjection = await api.startSearch(context'), 'Surrender must dismiss its server result before optional queued matchmaking.');
$surrenderStart = strpos($matchOwner, "if (action === 'surrender')");
$surrenderEnd = strpos($matchOwner, "if (action === 'dismiss-result')", $surrenderStart ?: 0);
$surrenderBranch = $surrenderStart !== false && $surrenderEnd !== false ? substr($matchOwner, $surrenderStart, $surrenderEnd - $surrenderStart) : '';
$assert($surrenderBranch !== '' && !str_contains($surrenderBranch, 'showPendingResult'), 'Manual surrender must not reuse the blocked result-wait screen.');
$assert(str_contains($matchOwner, 'showPendingResult') && str_contains($matchOwner, 'pendingMoveTitle'), 'Normal winning moves may still show an immediate result while the server confirms.');
...
$assert(str_contains($config, "build: 'mgw-clean-server-v4-action-priority'") && str_contains($config, 'MGW_CLEAN_RUNTIME_DATA_DIR'), 'Clean server must retain the exact action-priority staging build.');
