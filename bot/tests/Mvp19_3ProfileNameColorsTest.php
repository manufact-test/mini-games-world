...
$assertTrue(str_contains($apiSource, 'function publishCosmeticInventory') && str_contains($apiSource, "mgw:cosmetic-inventory-changed") && str_contains($apiSource, 'const next = { ...current, equipped:{ ...equipped } };') && str_contains($apiSource, 'state.profileInventory = next;'), 'Successful Store mutations must publish authoritative equipped cosmetics into shared client state');
...
$assertTrue(str_contains($nameColorCss, '.store-v2-name-color-foot{display:flex;flex-direction:column;align-items:stretch;justify-content:flex-end;gap:7px'), 'Store price and action button must keep a readable vertical gap');
$openProfileStart = strpos($profileSource, 'export function openProfile(event = null)');
$immediatePaint = is_int($openProfileStart) ? strpos($profileSource, 'showProfileImmediately();', $openProfileStart) : false;
$scheduledRefresh = is_int($immediatePaint) ? strpos($profileSource, 'scheduleProfileRefreshAfterEntry();', $immediatePaint) : false;
$refreshHelper = strpos($profileSource, 'function scheduleProfileRefreshAfterEntry()');
$backgroundRefresh = is_int($refreshHelper) ? strpos($profileSource, 'api.profileV2()', $refreshHelper) : false;
$assertTrue(is_int($openProfileStart) && is_int($immediatePaint) && is_int($scheduledRefresh) && is_int($refreshHelper) && is_int($backgroundRefresh) && $immediatePaint < $scheduledRefresh && str_contains($profileSource, 'function warmProfileSnapshot()'), 'Profile navigation must paint immediately from shared state, then converge through the scheduled authoritative refresh without competing with the first route frame');
...
$assertTrue(str_contains($mainCss, 'mgw-name-colors.css?v=4') && str_contains($mainCss, 'profile-corrective.css?v=7'), 'Main stylesheet must request the price-spacing name-color and accepted Tic Tac Toe tab presentation identities');
$assertTrue(str_contains($manifestSource, 'name-color-live-projection') && str_contains($manifestSource, 'name-color-chrome') && str_contains($manifestSource, 'compact-name-colors-instant-profile') && str_contains($manifestSource, 'perf=stable-render-cache'), 'Active v110 delivery graph must carry fresh compact-grid and stable-Profile render cache identities');
