...
$assertTrue(str_contains($wrapper, "const PREVIEW_BOARD_STYLE = 'position:absolute;left:8%;top:50%;width:84%;height:auto;display:grid;grid-template-columns:repeat(10,minmax(0,1fr));grid-template-rows:none;grid-auto-rows:auto;gap:2px;transform:translateY(-50%);z-index:2;'"), 'Battleship preview rows must be driven by 1:1 cells so browser track rounding cannot squash a row.');
...
$assertTrue(str_contains($wrapper, "classic:'background:#e7c56e;border:1px solid #ffe6a6") && str_contains($wrapper, "modern:'background:#4f7f96;border:1px solid #9ee9ff") && str_contains($wrapper, "armored:'background:#4b535b;border:1px solid #aab4bc") && str_contains($wrapper, "neon:'background:#4937a5;border:1px solid #59f6ff") && str_contains($wrapper, 'inset 0 0 0 1px rgba(255,68,222,.76)'), 'All fleet preview cells must render as full filled hull materials; Neon must visibly retain a violet body with cyan/magenta rims.');
...
$assertTrue(str_contains($outer, 'store-screen-battleship-store-v1.js?v=9&mvp19_12=store-preview-parity-v9&header=steel-ship&neon_frame=outer-safe&neon_fleet=tube-v4&fleet_preview=filled-parity-v2&preview_geometry=cell-driven-v4&hydration=observer-v1&inline_owner=v3&effects=unchanged-v2'), 'Active Store wrapper must install Battleship preview parity v9 with circular rows and filled fleet materials');
$activeStore = (string)($manifest['imports']['./assets/js/screens/store-screen.js?v=34'] ?? '');
$assertTrue(str_contains($activeStore, 'store-screen-checkers-board-source-wrapper.js?v=29') && str_contains($activeStore, 'battleship_store=preview-parity-v9') && str_contains($activeStore, 'battleship_geometry=square') && str_contains($activeStore, 'battleship_header=steel-ship-v1') && str_contains($activeStore, 'battleship_neon_frame=outer-safe-v1') && str_contains($activeStore, 'battleship_neon_fleet=tube-v4') && str_contains($activeStore, 'battleship_preview_geometry=cell-driven-v4') && str_contains($activeStore, 'battleship_store_hydration=observer-v1') && str_contains($activeStore, 'battleship_preview_inline_owner=v3') && str_contains($activeStore, 'battleship_fleet_preview=filled-parity-v2') && str_contains($activeStore, 'battleship_effects=distinct-v2'), 'Active Store graph must publish preview parity v9');
$assertTrue(str_contains($launch, 'v=1231') && str_contains($launch, 'battleship_store=preview-parity-v9') && str_contains($launch, 'battleship_header=steel-ship-v1') && str_contains($launch, 'battleship_neon_frame=outer-safe-v1') && str_contains($launch, 'battleship_neon_fleet=tube-v4') && str_contains($launch, 'battleship_preview_geometry=cell-driven-v4') && str_contains($launch, 'battleship_preview_inline_owner=v3') && str_contains($launch, 'battleship_fleet_preview=filled-parity-v2') && str_contains($launch, 'battleship_effects=distinct-v2'), 'Telegram launch must force preview parity v9');
