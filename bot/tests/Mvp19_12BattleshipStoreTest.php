...
$assertTrue(str_contains($css, 'grid-auto-rows:auto') && str_contains($css, 'transform:translateY(-50%)') && str_contains($css, 'aspect-ratio:1 / 1;'), 'Battleship Store/Profile 10x10 previews must keep every row/cell equally circular instead of compressing one row');
$assertTrue(str_contains($css, '.mgw-battleship-preview.fleet-neon') && str_contains($css, '#5f46d8') && str_contains($css, '#3a2a86') && str_contains($css, 'rgba(255,73,224,.58)') && str_contains($css, '#41f6ff'), 'Neon Fleet preview must match the clean filled LIVE neon material');
...
$assertTrue(str_contains($outer, 'store-screen-battleship-store-v1.js?v=4&mvp19_12=store-preview-parity-v4&header=steel-ship&neon_frame=outer-safe&neon_fleet=flat-v3&preview_geometry=equal-cells-v1&effects=unchanged-v2'), 'Active Store wrapper must install Battleship preview parity v4 without rewriting accepted effect preview code');
$activeStore = (string)($manifest['imports']['./assets/js/screens/store-screen.js?v=34'] ?? '');
$assertTrue(str_contains($activeStore, 'battleship_store=preview-parity-v4') && str_contains($activeStore, 'battleship_geometry=square') && str_contains($activeStore, 'battleship_header=steel-ship-v1') && str_contains($activeStore, 'battleship_neon_frame=outer-safe-v1') && str_contains($activeStore, 'battleship_neon_fleet=flat-v3') && str_contains($activeStore, 'battleship_preview_geometry=equal-cells-v1') && str_contains($activeStore, 'battleship_effects=distinct-v2'), 'Active Store graph must publish current preview parity while preserving effect identity');
$assertTrue(str_contains($launch, 'battleship_store=preview-parity-v4') && str_contains($launch, 'battleship_header=steel-ship-v1') && str_contains($launch, 'battleship_neon_frame=outer-safe-v1') && str_contains($launch, 'battleship_neon_fleet=flat-v3') && str_contains($launch, 'battleship_preview_geometry=equal-cells-v1') && str_contains($launch, 'battleship_effects=distinct-v2'), 'Telegram launch must publish Battleship Store/Profile preview parity without changing effect identity');
