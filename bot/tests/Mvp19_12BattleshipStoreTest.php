...
$assertTrue(str_contains($wrapper, "const PREVIEW_BOARD_STYLE = 'position:absolute;left:8%;top:50%;width:84%;height:auto;aspect-ratio:1 / 1;display:grid;grid-template-columns:repeat(10,minmax(0,1fr));grid-template-rows:repeat(10,minmax(0,1fr));gap:2px;transform:translateY(-50%);z-index:2;'"), 'Battleship preview geometry must be owned inline by the shared JS markup so no stylesheet cache can leave a squashed row.');
...
$assertTrue(str_contains($wrapper, "neon:'background:linear-gradient(145deg,#5a45bd 0%,#342778 58%,#171936 100%);border:1px solid #55f5ff") && str_contains($wrapper, 'inset 0 0 0 1px rgba(255,70,223,.62)') && str_contains($wrapper, "armored:'background:radial-gradient(circle at 30% 28%,rgba(255,255,255,.48) 0 6%,transparent 7%)"), 'Fleet preview materials must be scale-adapted so the LIVE hull material remains readable in tiny Store/Profile cells.');
...
$assertTrue(str_contains($outer, 'store-screen-battleship-store-v1.js?v=8&mvp19_12=store-preview-parity-v8&header=steel-ship&neon_frame=outer-safe&neon_fleet=tube-v4&fleet_preview=scale-parity-v1&preview_geometry=inline-square-v3&hydration=observer-v1&inline_owner=v2&effects=unchanged-v2'), 'Active Store wrapper must install Battleship preview parity v8 with scale-adapted fleet materials');
$activeStore = (string)($manifest['imports']['./assets/js/screens/store-screen.js?v=34'] ?? '');
$assertTrue(str_contains($activeStore, 'store-screen-checkers-board-source-wrapper.js?v=28') && str_contains($activeStore, 'battleship_store=preview-parity-v8') && str_contains($activeStore, 'battleship_geometry=square') && str_contains($activeStore, 'battleship_header=steel-ship-v1') && str_contains($activeStore, 'battleship_neon_frame=outer-safe-v1') && str_contains($activeStore, 'battleship_neon_fleet=tube-v4') && str_contains($activeStore, 'battleship_preview_geometry=inline-square-v3') && str_contains($activeStore, 'battleship_store_hydration=observer-v1') && str_contains($activeStore, 'battleship_preview_inline_owner=v2') && str_contains($activeStore, 'battleship_fleet_preview=scale-parity-v1') && str_contains($activeStore, 'battleship_effects=distinct-v2'), 'Active Store graph must publish fleet preview parity v8');
$assertTrue(str_contains($launch, 'v=1230') && str_contains($launch, 'battleship_store=preview-parity-v8') && str_contains($launch, 'battleship_header=steel-ship-v1') && str_contains($launch, 'battleship_neon_frame=outer-safe-v1') && str_contains($launch, 'battleship_neon_fleet=tube-v4') && str_contains($launch, 'battleship_preview_geometry=inline-square-v3') && str_contains($launch, 'battleship_preview_inline_owner=v2') && str_contains($launch, 'battleship_fleet_preview=scale-parity-v1') && str_contains($launch, 'battleship_effects=distinct-v2'), 'Telegram launch must force fleet preview parity v8');
