...
$assertTrue(str_contains($outer, 'store-screen-battleship-store-v1.js?v=3&mvp19_12=store-polish-v3&header=steel-ship&neon_frame=outer-safe&effects=unchanged-v2'), 'Active Store wrapper must install Battleship polish v3 without rewriting accepted effect preview code');
$activeStore = (string)($manifest['imports']['./assets/js/screens/store-screen.js?v=34'] ?? '');
$assertTrue(str_contains($activeStore, 'battleship_store=polish-v3') && str_contains($activeStore, 'battleship_geometry=square') && str_contains($activeStore, 'battleship_header=steel-ship-v1') && str_contains($activeStore, 'battleship_neon_frame=outer-safe-v1') && str_contains($activeStore, 'battleship_effects=distinct-v2'), 'Active Store graph must publish the header/neon polish while preserving the current effect identity');
$assertTrue(str_contains($launch, 'battleship_store=polish-v3') && str_contains($launch, 'battleship_header=steel-ship-v1') && str_contains($launch, 'battleship_neon_frame=outer-safe-v1') && str_contains($launch, 'battleship_effects=distinct-v2'), 'Telegram launch must publish Battleship Store polish v3 without changing effect identity');
