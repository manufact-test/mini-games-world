...
const wrapper = fs.readFileSync('app/assets/js/screens/store-screen-checkers-effects-wrapper.js', 'utf8');
const baseWrapper = fs.readFileSync('app/assets/js/screens/store-screen-checkers-wrapper.js', 'utf8');
...
ok(manifest.includes('store-screen-checkers-effects-wrapper.js?v=1&mvp19_6=effects-live-board-v1'), 'active Store route uses effect-only wrapper');
ok(wrapper.includes("store-screen-checkers-wrapper.js?v=4&mvp19_6=visual-corrective-v3"), 'effect wrapper delegates to existing Checkers Store owner');
ok(wrapper.includes('store-effects-live-board-v1.css?v=2&mvp19_6=effects-live-board-only'), 'effect wrapper loads cache-busted effects stylesheet');
ok(baseWrapper.includes('runBoundedEffectPreview'), 'existing bounded finite replay owner preserved');
ok(baseWrapper.includes("preview.classList.add('is-previewing')"), 'existing effect replay trigger preserved');
