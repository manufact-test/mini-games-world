...
if (!is_string($index) || !is_string($entry) || !is_string($main)) throw new RuntimeException('Missing v124 main entry source.');
$assertions = 0;
...
$assert(str_contains($entry, 'data-hotfix-build="v124-mvp14-d1-live-failure-fixes"')
    && str_contains($entry, 'X-MGW-Frontend-Build: v124-mvp14-d1-live-failure-fixes'), 'The served shell and response header must publish v124.');
...
$assert(substr_count($entry, 'notification-window-owner-v124.js?v=124') === 1
    ...
    && !str_contains($entry, 'notification-window-owner-v119.js?v=119'), 'The cache-busted shell must publish v124 notification ownership.');
