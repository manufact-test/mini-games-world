...
$assert(substr_count($index, './assets/js/main.js?v=98.3') === 1
    ...
    && str_contains($entry, "'./assets/js/main.js?v=d1-canonical-toast-seed'"),
    'Source shell must retain one replacement anchor and staging must publish canonical main.');
$assert(str_contains($entry, 'data-hotfix-build="d1-canonical-toast-seed"')
    && str_contains($entry, 'X-MGW-Frontend-Build: d1-canonical-toast-seed')
    && str_contains($main, "window.__MGW_BUILD__ = 'd1-canonical-toast-seed'"),
