...
$assert(is_string($health)
    ...
    && str_contains($health, "\$databaseStatus['failure'] = DatabaseFailureClassifier::classify(\$databaseError);")
    && str_contains($health, "\$databaseStatus['failure'] = null;"),
    'Health must publish the safe classifier result through its existing database check.');
$assert(is_string($classifier)
    && str_contains($classifier, "'category' =>")
    && str_contains($classifier, "'sqlstate' =>")
    && str_contains($classifier, "'driver_code' =>")
    && !str_contains($classifier, "'message' =>")
    && !str_contains($classifier, "'dsn' =>")
    && !str_contains($classifier, "'user' =>")
    && !str_contains($classifier, "'password' =>"),
    'Public diagnostics must expose only a safe category and numeric database codes.');
