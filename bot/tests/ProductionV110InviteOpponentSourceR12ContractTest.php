...
$assert(str_contains($endpoint, "\$users = is_array(\$data['users'] ?? null) ? \$data['users'] : [];")
    && str_contains($endpoint, 'foreach ($users as $candidateId => $candidate)')
    && str_contains($endpoint, '$hasHistory = isset($lastGameAt[$candidateId]);')
    && str_contains($endpoint, '86400 * 30'),
    'The canonical JSON catalog must include online and recently known human players instead of only finished-match opponents.');
...
$assert(str_contains($endpoint, '$onlineOpponentIds')
    ...
    'The endpoint must prove that observed online opponents resolve to canonical profiles before final empty.');
