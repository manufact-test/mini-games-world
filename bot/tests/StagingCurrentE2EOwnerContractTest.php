...
$currentSpec = file_get_contents($root . '/e2e/staging/current-core-live.spec.mjs');
...
$assert(str_contains($currentConfig, "testMatch: 'current-core-live.spec.mjs'")
    && !str_contains($currentConfig, 'supersededScenarios'),
    'Blocking staging Playwright must run only the current live core.');
$assert(str_contains($legacyConfig, "testIgnore: ['current-core-live.spec.mjs']")
    && str_contains($legacyConfig, 'supersededScenarios'),
    'Historical staging acceptance and superseded current-core iterations must remain in the legacy config.');
...
$assert(str_contains($currentSpec, "'x-mgw-client-bootstrap'")
    ...
    'Current live core must prove the v110 bootstrap and unified game-zone owners.');
...
$assert(str_contains($currentSpec, 'A.bootstrap.match_economy.entry_cost')
    ...
    'Current live core must use server-owned entry cost instead of stale fixtures.');
$assert(str_contains($currentSpec, 'A.profile.user.balance')
    && !str_contains($currentSpec, 'balance_match'),
    'Current live core must validate unified balance rather than retired balance_match.');
$assert(str_contains($currentSpec, 'async function firstUiTap(')
    ...
    'Current live core must prove the first TTT UI tap commits on its first click.');
$assert(str_contains($currentSpec, "'/bot/invites.php', { action: 'sync' }")
    ...
    'Direct invite receipt must follow the active v110 invite_events owner, not a separate Notification Center read.');
$assert(str_contains($currentSpec, "action: 'create_direct'")
    ...
    'Current live core must exercise the real two-player direct invitation lifecycle.');
$assert(str_contains($currentSpec, "action: 'history'")
    ...
    'Current live core must prove Result/History economy consistency from the authoritative ledger projection.');
$assert(str_contains($currentSpec, "'/bot/presence.php'")
    && str_contains($currentSpec, 'serverErrors'),
    'Current live core must surface current presence/server 5xx responses.');
$assert(str_contains($currentSpec, 'await resetPlayers();'),
    'Current live core must self-clean A/B state after the browser scenario.');
