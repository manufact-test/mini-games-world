...
if (!is_string($entry) || !is_string($notifications) || !is_string($background)
    || !is_string($opponents) || !is_string($endpoint)) throw new RuntimeException('Missing integrated D1 v124 sources.');
$assertions = 0;
...
$assert(substr_count($entry, 'notification-window-owner-v124.js?v=124') === 1
    ...
    && !str_contains($entry, 'notifications-passive-v121.js?v=121'), 'v124 must publish one owner and passive service.');
$assert(substr_count($entry, 'opponents-authoritative-confirm-v122.js?v=122') === 1
    && !str_contains($entry, 'opponents-authoritative-confirm-v117.js?v=117'), 'v124 must publish v122 opponent confirmation once.');
...
$assert(!str_contains($notifications, 'openingSheet')
    ...
    && str_contains($notifications, "window.addEventListener('touchend'"), 'Notification input and responses must share generation protection.');
$assert(!str_contains($background, 'openNotificationsSheet(')
    ...
    && str_contains($background, 'rememberBaselineNotifications(items, requestStartedAt)'), 'The background service must remain passive and preserve in-flight fresh notifications.');
...
$assert(str_contains($entry, 'data-hotfix-build="v124-mvp14-d1-live-failure-fixes"')
    && str_contains($entry, 'X-MGW-Frontend-Build: v124-mvp14-d1-live-failure-fixes'), 'The no-cache entry must expose v124.');
