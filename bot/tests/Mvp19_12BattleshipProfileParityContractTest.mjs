...
assert.ok(profile.includes("store-screen-battleship-store-v1.js?v=9&mvp19_12=store-preview-parity-v9&header=steel-ship&neon_frame=outer-safe&neon_fleet=tube-v4&fleet_preview=filled-parity-v2&preview_geometry=cell-driven-v4&hydration=observer-v1&inline_owner=v3&effects=unchanged-v2"), 'Profile must import the exact current Store preview owner');
...
assert.ok(store.includes("const PREVIEW_BOARD_STYLE = 'position:absolute;left:8%;top:50%;width:84%;height:auto;display:grid;grid-template-columns:repeat(10,minmax(0,1fr));grid-template-rows:none;grid-auto-rows:auto;gap:2px;transform:translateY(-50%);z-index:2;'"), 'Store/Profile previews must share the cell-driven circular-row geometry owner');
assert.ok(store.includes("neon:'background:#4937a5;border:1px solid #59f6ff") && store.includes('inset 0 0 0 1px rgba(255,68,222,.76)') && store.includes("classic:'background:#e7c56e;border:1px solid #ffe6a6"), 'Profile must reuse the filled fleet materials from the shared Store owner');
...
assert.ok(layout.includes("mgw-profile-battleship-parity.js?v=9&mvp19_12=profile-four-parity-v3&store=preview-parity-v9&geometry=square&header=steel-ship&neon_fleet=tube-v4&fleet_preview=filled-parity-v2&preview_geometry=cell-driven-v4&hydration=observer-v1&inline_owner=v3&effects=unchanged-v2&copy=four-pattern"), 'Active Profile owner must import Battleship Four-parity v3 with preview parity v9');
...
assert.ok(profileOwnerMatch && Number(profileOwnerMatch[1]) >= 36, 'Active Profile owner must bump the parent layout cache so preview parity v9 reaches clients');
assert.ok(manifest.includes('mgw-profile-chess-layout-v2.js?v=36') && manifest.includes('battleship_profile=four-parity-v3') && manifest.includes('battleship_store=preview-parity-v9') && manifest.includes('battleship_header=steel-ship-v1') && manifest.includes('battleship_neon_frame=outer-safe-v1') && manifest.includes('battleship_neon_fleet=tube-v4') && manifest.includes('battleship_preview_geometry=cell-driven-v4') && manifest.includes('battleship_preview_inline_owner=v3') && manifest.includes('battleship_fleet_preview=filled-parity-v2'), 'Manifest must bump the parent Profile owner and publish preview parity v9');
