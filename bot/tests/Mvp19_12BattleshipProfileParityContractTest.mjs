...
assert.ok(profile.includes("store-screen-battleship-store-v1.js?v=8&mvp19_12=store-preview-parity-v8&header=steel-ship&neon_frame=outer-safe&neon_fleet=tube-v4&fleet_preview=scale-parity-v1&preview_geometry=inline-square-v3&hydration=observer-v1&inline_owner=v2&effects=unchanged-v2"), 'Profile must import the exact current Store preview owner');
...
assert.ok(store.includes("const PREVIEW_BOARD_STYLE = 'position:absolute;left:8%;top:50%;width:84%;height:auto;aspect-ratio:1 / 1;display:grid;grid-template-columns:repeat(10,minmax(0,1fr));grid-template-rows:repeat(10,minmax(0,1fr));gap:2px;transform:translateY(-50%);z-index:2;'"), 'Store/Profile previews must share the inline square 10x10 geometry owner');
assert.ok(store.includes("neon:'background:linear-gradient(145deg,#5a45bd 0%,#342778 58%,#171936 100%);border:1px solid #55f5ff") && store.includes('inset 0 0 0 1px rgba(255,70,223,.62)'), 'Profile must reuse the scale-adapted Neon Fleet material from the shared Store owner');
assert.ok(storeCss.includes('#4d3aaa') && storeCss.includes('#2a205f') && storeCss.includes('#55f5ff') && storeCss.includes('inset 0 0 0 2px rgba(255,70,223,.34)'), 'Store/Profile Neon Fleet must match the current LIVE luminous-rim material');
...
assert.ok(layout.includes("mgw-profile-battleship-parity.js?v=8&mvp19_12=profile-four-parity-v3&store=preview-parity-v8&geometry=square&header=steel-ship&neon_fleet=tube-v4&fleet_preview=scale-parity-v1&preview_geometry=inline-square-v3&hydration=observer-v1&inline_owner=v2&effects=unchanged-v2&copy=four-pattern"), 'Active Profile owner must import Battleship Four-parity v3 with fleet preview parity v8');
...
assert.ok(profileOwnerMatch && Number(profileOwnerMatch[1]) >= 35, 'Active Profile owner must bump the parent layout cache so fleet preview parity v8 reaches clients');
assert.ok(manifest.includes('mgw-profile-chess-layout-v2.js?v=35') && manifest.includes('battleship_profile=four-parity-v3') && manifest.includes('battleship_store=preview-parity-v8') && manifest.includes('battleship_header=steel-ship-v1') && manifest.includes('battleship_neon_frame=outer-safe-v1') && manifest.includes('battleship_neon_fleet=tube-v4') && manifest.includes('battleship_preview_geometry=inline-square-v3') && manifest.includes('battleship_preview_inline_owner=v2') && manifest.includes('battleship_fleet_preview=scale-parity-v1'), 'Manifest must bump the parent Profile owner and publish fleet preview parity v8');
