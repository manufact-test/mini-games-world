...
if (!is_string($entry) || !is_string($owner)) {
    throw new RuntimeException('Missing canonical desktop notification input sources.');
}
...
$assert(!str_contains($entry, 'notification-window-owner-v119.js?v=119')
    ...
    'Retired click-only and retry owners must not compete with v121.');
$assert(str_contains($owner, "window.addEventListener('pointerdown'")
    ...
    'The original desktop pointer sequence must be owned at window capture.');
$assert(str_contains($owner, 'performance.now() - pointer.startedAt')
    ...
    'A short stationary pointerup must open immediately from the real gesture.');
$assert(str_contains($owner, 'if (Date.now() <= suppressClickUntil')
        && !str_contains($owner, '.click()')
        && !str_contains($owner, 'openingSheet'),
    'The generated click must be consumed without a synthetic retry or opening lock.');
$assert(str_contains($owner, 'renderLoading();')
    ...
    'Slow and empty responses must remain independently generation-gated.');
