...
$assertTrue(str_contains($wrapper, 'production-v111-profile-avatar-geometry.css?v=2') && str_contains($wrapper, 'ensureAvatarGeometryStylesheet()'), 'Profile-only final geometry owner must be cache-published after older parity layers');
...
$assertTrue(str_contains($manifest, 'mgw-profile-victory-effects-card-parity.js?v=6&mvp19_3=profile-final-subtitle-namecolor&visual_repair=6') && str_contains($manifest, 'victory=spark-burst-profile-final-subtitle-namecolor&visual_repair=6'), 'Active v110 manifest must cache-publish the final Profile subtitle/name-colour corrective');
