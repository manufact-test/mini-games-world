...
$assertTrue(str_contains($wrapper, 'production-v111-profile-avatar-geometry.css?v=1') && str_contains($wrapper, 'ensureAvatarGeometryStylesheet()'), 'Profile-only avatar geometry owner must load after the older parity layers');
...
$assertTrue(str_contains($cardParityCss, "content:'Эффект входа'!important") && str_contains($cardParityCss, "content:'Эффект победы'!important"), 'Profile Entry/Victory cards must show family subtitles instead of tier levels');
...
$assertTrue(str_contains($profileGeometryCss, '[data-profile-avatar-preview]::after') && str_contains($profileGeometryCss, 'content:none!important') && str_contains($profileGeometryCss, 'display:none!important'), 'Profile avatars must no longer render redundant Starter/Avatar subtitles');
...
$assertTrue(str_contains($profileGeometryCss, '.profile-v2-entry-effect-copy') && str_contains($profileGeometryCss, '.profile-v2-victory-effect-copy') && str_contains($profileGeometryCss, 'min-height:0!important') && str_contains($profileGeometryCss, 'margin:auto var(--mgw-profile-copy-inset) 0!important'), 'Entry/Victory copy must keep text visible at the same bottom rhythm as avatar cards');
...
$assertTrue(str_contains($manifest, 'mgw-profile-victory-effects-card-parity.js?v=5&mvp19_3=profile-avatar-visible-geometry&visual_repair=5') && str_contains($manifest, 'victory=spark-burst-profile-avatar-visible-geometry&visual_repair=5'), 'Active v110 manifest must cache-publish the final Profile avatar-geometry owner');
