...
$assertTrue(str_contains($wrapper, 'initBaseVictoryEffects()') && str_contains($wrapper, 'production-v110-victory-effects-card-parity.css?v=2'), 'Victory card parity must wrap the accepted live owner and cache-publish the corrective');
$assertTrue(str_contains($cardParityCss, '.store-v2-entry-effect-copy') && str_contains($cardParityCss, 'margin:5px 1px 0!important') && str_contains($cardParityCss, 'height:auto!important'), 'Entry and Victory Store copy must mirror the accepted avatar title geometry');
$assertTrue(str_contains($cardParityCss, '.store-v2-entry-effect-card>.mgw-profile-cosmetic-foot') && str_contains($cardParityCss, 'margin-top:4px!important') && str_contains($cardParityCss, 'padding-top:0!important') && str_contains($cardParityCss, 'gap:5px!important'), 'Entry and Victory Store footers must mirror the accepted avatar footer rhythm');
$assertTrue(str_contains($cardParityCss, '#screen-profile .profile-v2-reaction-card>strong') && str_contains($cardParityCss, 'font-size:8.4px!important') && str_contains($cardParityCss, 'font-size:7.3px!important'), 'Reaction, Entry and Victory Profile card typography must mirror accepted avatar title/subtitle sizes');
$assertTrue(str_contains($cardParityCss, '[data-profile-victory-effect-collection]>.profile-v2-collection-title') && str_contains($cardParityCss, 'font-size:11px!important'), 'Reaction, Entry and Victory Profile section headings must keep the avatar collection heading token');
...
$assertTrue(str_contains($manifest, 'mgw-profile-victory-effects-card-parity.js?v=2&mvp19_3=exact-avatar-parity-v4') && str_contains($manifest, 'victory=spark-burst-exact-avatar-parity-v4'), 'Active v110 manifest must cache-publish exact avatar spacing/Profile typography parity');
