...
$assertTrue(str_contains($frameSource, 'FRAME_DISPLAY_NAMES') && str_contains($frameSource, "'profile-frame-01':'Голубое небо'") && str_contains($frameSource, "'profile-frame-02':'Золотой ореол'") && str_contains($frameSource, "'profile-frame-03':'Аврора'") && str_contains($frameSource, "'profile-frame-animated':'Живой спектр'"), 'Frame presentation must expose concise user-facing product names instead of placeholder labels');
...
$assertTrue(str_contains($frameCss, "mgw-avatar-characters-v2.webp?v=1") && str_contains($frameCss, '.mgw-profile-frame-preview>span'), 'Store and Profile frame previews must show the canonical starter portrait inside the frame');
...
$assertTrue(str_contains($cleanEntry, 'initMgwProfileFrames') && str_contains($cleanEntry, 'mgw-profile-frames.js?v=3&mvp19_3=profile-frame-name-polish'), 'Active clean entry must initialize the polished frame-name build');
...
$assertTrue(str_contains($mainCss, 'mgw-profile-frames.css?v=2&mvp19_3=profile-frame-preview-polish') && str_contains($manifest, 'mvp19_3_10=profile-frame-name-polish') && str_contains($manifest, 'mvp19_3_7=profile-v2-read-coalesce'), 'Active delivery graph must carry polished frame names and coalesced Profile v2 client identities');
