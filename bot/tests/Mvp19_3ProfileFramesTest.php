...
$assertTrue(str_contains($frameSource, 'FRAME_DISPLAY_NAMES') && str_contains($frameSource, "'profile-frame-01':'Небо'") && str_contains($frameSource, "'profile-frame-02':'Золото'") && str_contains($frameSource, "'profile-frame-03':'Аврора'") && str_contains($frameSource, "'profile-frame-animated':'Спектр'"), 'Frame presentation must expose concise user-facing names instead of placeholder Roman numerals');
...
$assertTrue(str_contains($cleanEntry, 'initMgwProfileFrames') && str_contains($cleanEntry, 'mgw-profile-frames.js?v=2&mvp19_3=profile-frame-preview-polish'), 'Active clean entry must initialize polished profile frames');
...
$assertTrue(str_contains($mainCss, 'mgw-profile-frames.css?v=2&mvp19_3=profile-frame-preview-polish') && str_contains($manifest, 'mvp19_3_8=profile-frame-preview-polish') && str_contains($manifest, 'mvp19_3_7=profile-v2-read-coalesce'), 'Active delivery graph must carry fresh polished profile-frame JS/CSS and coalesced Profile v2 client identities');
