...
$showcaseCss = (string)file_get_contents($root . '/app/assets/css/production-v102-entry-effects-showcase.css');
...
$assertTrue(str_contains($entryCss, '@media(prefers-reduced-motion:reduce)') && str_contains($entryCss, 'pointer-events:none'), 'Entry Effect base presentation must remain reduced-motion safe and non-blocking');
...
$assertTrue(str_contains($acceptanceCss, 'padding:12px !important'), 'Every Profile cosmetic card must use the canonical 12px inset on all four sides');
...
$assertTrue(str_contains($showcaseCss, 'mgwEntryLegendRealSword') && str_contains($showcaseCss, 'mgwEntryLegendArcImpact') && str_contains($showcaseCss, 'clip-path:polygon'), 'Legendary Entry Effect must render an actual sword silhouette plus a curved impact arc instead of decorative gold bars');
$assertTrue(str_contains($showcaseCss, '[data-entry-effect-variant="entry-02"]') && str_contains($showcaseCss, 'width:154px !important') && str_contains($showcaseCss, 'height:190px !important'), 'Mid-tier portal must feature a substantial fantasy character instead of a tiny badge');
...
$assertTrue(str_contains($manifest, 'mgw-profile-entry-effects.js?v=5&mvp19_3=launch-overlay-gated') && str_contains($manifest, 'entry_effect_handoff=1') && str_contains($manifest, 'profile-bottom-spacing-parity-v3') && str_contains($manifest, 'profile_card_inset=12') && str_contains($manifest, 'entry_visual=fantasy-showcase-v3') && str_contains($manifest, 'purchase=insufficient-explicit-v1') && str_contains($manifest, 'production-v102-entry-effects-showcase.css'), 'Active v110 manifest must cache-publish launch-gated Entry Effects, canonical Profile spacing, showcase visuals and explicit insufficient-funds UX');
