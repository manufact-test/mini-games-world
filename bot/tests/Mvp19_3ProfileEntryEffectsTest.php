...
$assertTrue(str_contains($acceptanceCss, 'mgw-avatar-characters-v2.webp') && str_contains($acceptanceCss, 'mgwEntryLiveSword') && str_contains($acceptanceCss, 'mgwEntryLiveImpact'), 'Legendary Tier III must use existing raster character art plus a sword strike and impact presentation');
...
$assertTrue(str_contains($manifest, 'mgw-profile-entry-effects.js?v=5&mvp19_3=launch-overlay-gated') && str_contains($manifest, 'entry_effect_handoff=1') && str_contains($manifest, 'profile-bottom-spacing-parity-v3') && str_contains($manifest, 'profile_card_inset=12') && str_contains($manifest, 'entry_visual=tiered-v2'), 'Active v110 manifest must cache-publish launch-gated Entry Effects plus canonical 12px Profile spacing and tiered visuals');
