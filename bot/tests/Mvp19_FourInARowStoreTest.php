...
$assertTrue(str_contains($outer, "store-screen-four-in-a-row-store-v1.js?v=3&four_store=static-v3"), 'Active Store wrapper must install Four in a Row presentation');
...
$assertTrue(str_contains($activeStore, 'four_store=static-v3'), 'Active Store graph must publish static Four in a Row phase');
$launchMatch = [];
...
$assertTrue(str_contains($css, '.store-v2-game-product[data-store-game-product="four_in_a_row"]') && str_contains($css, 'width:92%'), 'Compact Four in a Row element cards must enlarge discs without changing the purchase sheet size');
