...
$assertTrue(str_contains($badgeSource, "const BADGE_PREVIEW_AVATAR = 'starter-default-01'") && str_contains($badgeSource, 'mgw-profile-badge-demo-avatar') && str_contains($badgeSource, 'style="border-radius:24%"'), 'Store/Profile badge previews must use the canonical starter portrait in the same rounded-square avatar shape as normal identity cards');
...
$assertTrue(str_contains($cleanEntry, 'initMgwProfileBadges') && str_contains($cleanEntry, 'mgw-profile-badges.js?v=5&mvp19_3=profile-badge-avatar-shape'), 'Active v110 clean entry must initialize the rounded-square badge avatar build');
$assertTrue(str_contains($mainCss, 'mgw-profile-badges.css?v=3&mvp19_3=profile-badge-avatar-demo') && str_contains($manifest, 'mvp19_3_11=profile-badge-avatar-shape'), 'Active v110 delivery graph must publish the badge avatar-shape identity without disturbing the existing CSS owner');
