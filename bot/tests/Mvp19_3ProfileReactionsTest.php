...
$assertTrue(str_contains($manifest, 'mgw-profile-reactions-header.js?v=5&mvp19_3=header-square-smooth&mobile=stable-bubble-canonical-profile-nav&profile_nav=canonical-pointer-v2') && str_contains($manifest, 'mgw-profile-reactions.js?v=6&mvp19_3=cumulative-owned-reactions&store=passive-owned&preview=bounded-packs-v2&route_work=game-only-v1') && str_contains($manifest, 'router.js?v=32&profile_nav=direct-sync-v2') && str_contains($manifest, 'production-v99-profile-reaction-polish.css?v=2&mvp19_3=cumulative-owned-reactions') && str_contains($manifest, 'main-v110-handoff-shell.js?v=1156&mvp18=friend-request-lifecycle&store=post-boot-warm&profile=boot-prepared-direct-route-v1&profile_first=background-raster-prewarm-v1') && str_contains($manifest, 'store_first=sync-prerender-v2') && str_contains($manifest, 'shell_motion=atomic-restored-v2') && str_contains($manifest, 'entry=paint-cached-first-v1'), 'Cumulative reactions and the accepted lag-free mobile shell must be cache-published without changing Profile preparation');
