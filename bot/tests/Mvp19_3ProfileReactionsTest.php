...
$assertTrue(str_contains($reactionUi, "document.addEventListener('pointerdown', primeProfileNavPressFeedback, true)") && str_contains($reactionUi, 'function primeProfileNavPressFeedback(event)') && str_contains($reactionUi, '#appBottomNav [data-shell-nav="profile"]') && str_contains($reactionUi, '}, 700);'), 'Historical Profile pointerdown feedback helper remains in the accepted reaction base and must be neutralized only at the mobile boundary');
...
$assertTrue(str_contains($reactionHeader, "window.addEventListener('pointerdown', suppressLegacyMobileProfilePressFeedback, true)") && str_contains($reactionHeader, 'event.stopPropagation();') && !str_contains($reactionHeader, 'handleMobileShellNavigation') && !str_contains($reactionHeader, "new CustomEvent('mgw:open-profile')"), 'Mobile Profile must use the canonical shell click path without phased capture navigation or synthetic route dispatch');
...
$assertTrue(str_contains($reactionPerfCss, '#app #screen-profile.screen') && str_contains($reactionPerfCss, 'transform:translateY(10px)!important') && str_contains($reactionPerfCss, 'transition:opacity .24s var(--sk-ease-standard),transform .24s var(--sk-ease-standard)!important') && !str_contains($reactionPerfCss, '#screen-profile{\n    transition:none !important') && str_contains($reactionPerfCss, 'backdrop-filter:none !important'), 'Profile must use the same shell rise transition on desktop/mobile while removing expensive mobile live blur instead of disabling route motion');
...
$assertTrue(str_contains($manifest, 'mgw-profile-reactions-header.js?v=4&mvp19_3=header-square-smooth&mobile=stable-bubble-canonical-profile-nav') && str_contains($manifest, 'mgw-profile-reactions.js?v=4&mvp19_3=ingame-smooth-corrective&first_frame=primed-v4&profile_nav=press-feedback-v1') && str_contains($manifest, 'main-v110-handoff-shell.js?v=1154&mvp18=friend-request-lifecycle&store=post-boot-warm') && str_contains($manifest, 'production-v98-mobile-profile-perf.css?v=4&mvp19_3=reaction-header-square-smooth') && str_contains($manifest, 'profile5=route-transition-parity-v1') && str_contains($manifest, 'header=metal-parity-v2') && str_contains($manifest, 'entry=paint-cached-first-v1'), 'Stable reaction host and canonical Profile transition parity must be cache-published without changing the shared shell owner');
