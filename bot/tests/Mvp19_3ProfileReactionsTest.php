...
$storeOpenPos = strpos($shell, 'const opening = openStoreTab();');
$storeRoutePos = $storeOpenPos === false ? false : strpos($shell, "showScreen('store');", $storeOpenPos);
$assertTrue($storeOpenPos !== false && $storeRoutePos !== false && $storeRoutePos > $storeOpenPos && !str_contains($shell, 'showStoreWhenFirstPresentationReady') && !str_contains($shell, 'storeFirstPresentationPromise') && !str_contains($shell, 'storeFirstPresentationReady'), 'First Store navigation must synchronously prepare its surface and publish the route without waiting for a network promise');
