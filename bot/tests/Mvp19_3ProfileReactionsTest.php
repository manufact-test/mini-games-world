...
$assertTrue(str_contains($reactionUi, "document.addEventListener('pointerdown', primeProfileNavPressFeedback, true)") && str_contains($reactionUi, 'function primeProfileNavPressFeedback(event)') && str_contains($reactionUi, '#appBottomNav [data-shell-nav="profile"]') && str_contains($reactionUi, '}, 700);'), 'Profile bottom-nav press feedback must paint before the existing synchronous route click and safely roll back if navigation is cancelled');
...
$assertTrue(str_contains($reactionHeader, "window.addEventListener('click', handleMobileShellNavigation, true)") && str_contains($reactionHeader, "route !== 'profile'") && substr_count($reactionHeader, 'window.requestAnimationFrame(() => {') >= 2 && str_contains($reactionHeader, "new CustomEvent('mgw:open-profile')"), 'Mobile Profile bottom-nav entry must guarantee a painted selection frame before opening the heavy Profile route');
...
$assertTrue(str_contains($manifest, 'mgw-profile-reactions-header.js?v=3&mvp19_3=header-square-smooth&mobile=stable-bubble-profile-phased-tap') && str_contains($manifest, 'mgw-profile-reactions.js?v=4&mvp19_3=ingame-smooth-corrective&first_frame=primed-v4&profile_nav=press-feedback-v1') && str_contains($manifest, 'main-v110-handoff-shell.js?v=1154&mvp18=friend-request-lifecycle&store=post-boot-warm') && str_contains($manifest, 'production-v98-mobile-profile-perf.css?v=3&mvp19_3=reaction-header-square-smooth') && str_contains($manifest, 'header=metal-parity-v2') && str_contains($manifest, 'profile4=paint-cached-first-v5') && str_contains($manifest, 'entry=paint-cached-first-v1'), 'Mobile stable reaction host and phased Profile entry must be cache-published without changing the shared shell owner');
