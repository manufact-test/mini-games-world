...
$storeRoutePos = strpos($shell, "showScreen('store');");
$storeLazyPos = $storeRoutePos === false ? false : strpos($shell, 'void openStoreTabLazy()', $storeRoutePos);
$assertTrue($storeRoutePos !== false && $storeLazyPos !== false && $storeLazyPos > $storeRoutePos && str_contains($shell, 'warmStoreScreenAfterFirstPaint();') && !str_contains($shell, 'showStoreWhenFirstPresentationReady') && !str_contains($shell, 'storeFirstPresentationPromise') && !str_contains($shell, 'storeFirstPresentationReady'), 'First Store navigation must publish the shell route synchronously while the accepted lazy Store graph resolves off the route change');
$assertTrue(str_contains($reactionPolishCss, '.screen[data-screen="tournaments"]') && str_contains($reactionPolishCss, '.screen[data-screen="store"]') && str_contains($reactionPolishCss, 'transition: none !important;') && !str_contains($reactionPolishCss, 'mgw-shell-content-rise-in'), 'Mobile Tournament and Store must keep the manually accepted atomic shell transition without whole-content compositor animation');
$assertTrue(str_contains($manifest, 'mgw-profile-reactions-header.js?v=5&mvp19_3=header-square-smooth&mobile=stable-bubble-canonical-profile-nav&profile_nav=canonical-pointer-v2') && str_contains($manifest, 'mgw-profile-reactions.js?v=6&mvp19_3=cumulative-owned-reactions&store=passive-owned&preview=bounded-packs-v2&route_work=game-only-v1') && str_contains($manifest, 'router.js?v=32&profile_nav=direct-sync-v2') && str_contains($manifest, 'production-v99-profile-reaction-polish.css?v=2&mvp19_3=cumulative-owned-reactions') && str_contains($manifest, 'main-v110-handoff-shell.js?v=1157&mvp18=friend-request-lifecycle&store=post-boot-warm&profile=boot-prepared-direct-route-v1') && str_contains($manifest, 'store_first=sync-prerender-v2') && str_contains($manifest, 'shell_motion=atomic-restored-v2') && str_contains($manifest, 'entry=paint-cached-first-v1') && str_contains($manifest, 'profile_first=covered-raster-prewarm-v2'), 'Cumulative reactions and the accepted lag-free mobile shell must publish the covered Profile first-raster preparation');
