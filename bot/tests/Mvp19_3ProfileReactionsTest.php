...
$mainShell = (string)file_get_contents($root . '/app/assets/js/main-v110-handoff-shell.js');
...
$assertTrue(str_contains($mainShell, 'function primeProfileNavigation(target)') && str_contains($mainShell, 'profileNavigationEpoch') && str_contains($mainShell, "document.dispatchEvent(new CustomEvent('mgw:open-profile'))"), 'Profile bottom-nav feedback must be projected before deferred route activation');
$assertTrue(str_contains($manifest, 'mgw-profile-reactions.js?v=4&mvp19_3=ingame-smooth-corrective&first_frame=primed-v4') && str_contains($manifest, 'main-v110-handoff-shell.js?v=1155') && str_contains($manifest, 'profile-nav=paint-before-open-v1') && str_contains($manifest, 'production-v98-mobile-profile-perf.css?v=3&mvp19_3=reaction-header-square-smooth') && str_contains($manifest, 'header=metal-parity-v2') && str_contains($manifest, 'profile4=paint-cached-first-v5') && str_contains($manifest, 'entry=paint-cached-first-v1'), 'Corrected first reaction, Profile tap phasing and accepted header/Profile assets must be cache-published through the active manifest');
