...
if (!is_string($entry) || !is_string($owner)) {
    throw new RuntimeException('Missing canonical mobile notification v121 sources.');
}
...
$assert(!str_contains($entry, 'notification-window-owner-v119.js?v=119')
    ...
    'Retired mobile and click-only owners must not remain active.');
$assert(str_contains($owner, '#notificationsOpen, #notificationToast')
    ...
    'The same canonical pointer owner must handle mobile and desktop surfaces.');
...
$assert(str_contains($owner, "document.addEventListener('mgw:notification-sync'")
    ...
    'The first frame must use live cache and the original mobile pointerup without a synthetic click.');
