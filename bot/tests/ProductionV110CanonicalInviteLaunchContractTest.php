...
$assert(
    WebAppLaunchUrl::base($config) === 'https://example.test/app/v110.php?v=110',
    'Canonical base URL must select the active v110 Telegram entrypoint.'
);

$assert(
    WebAppLaunchUrl::invitation($config, $token)
        === 'https://example.test/app/v110.php?v=110&invite=' . $normalizedToken,
    'Canonical invitation URL must append one normalized token to the v110 entrypoint.'
);

$assert(
    WebAppLaunchUrl::invitation($config, 'not-a-token')
        === 'https://example.test/app/v110.php?v=110',
...
$assert(
    ...
    'Start/menu and invite-message backends must depend on the same URL builder exactly once.'
);
...
$assert(
    ...
    'The active v110 client must accept both Telegram start_param and canonical invite query tokens through one open_link action.'
);
...
$assert(
    ...
    'Canonical invite launches must reach the current no-store v110 build.'
);
