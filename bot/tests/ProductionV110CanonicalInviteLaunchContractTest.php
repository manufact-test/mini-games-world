...
$assert(
    WebAppLaunchUrl::base($config) === 'https://example.test/app/v110.php?v=1109',
    'Canonical base URL must select the fresh production v1109 Telegram entrypoint.'
);
$assert(
    WebAppLaunchUrl::invitation($config, $token)
        === 'https://example.test/app/v110.php?v=1109&invite=' . $normalizedToken,
...
$assert(
    WebAppLaunchUrl::invitation($config, 'not-a-token')
        === 'https://example.test/app/v110.php?v=1109',
...
$assert(
    str_contains($v110, 'production-clean-entry-v110.js?v=1109')
        && str_contains($v110, 'main-v110.js?v=1109')
        && str_contains($v110, 'data-hotfix-build="v110-mvp14r4-invite-notification-presence-root"')
