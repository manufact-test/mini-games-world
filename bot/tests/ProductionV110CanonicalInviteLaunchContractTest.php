...
$assert(
    WebAppLaunchUrl::base($config) === 'https://example.test/app/v110.php?v=1119',
    'Canonical base URL must select the current v1119 Telegram entrypoint.'
);
$assert(
    WebAppLaunchUrl::invitation($config, $token)
        === 'https://example.test/app/v110.php?v=1119&invite=' . $normalizedToken,
    'Canonical invitation URL must append one normalized token to the current entrypoint.'
);
$assert(
    WebAppLaunchUrl::invitation($config, 'not-a-token')
        === 'https://example.test/app/v110.php?v=1119',
...
$assert(
    ...
    'Start/menu and invite-message backends must depend on the shared current URL builder exactly once.'
);
$assert(
...
$assert(
    str_contains($v110, 'production-clean-entry-v110.js?v=1119')
        && str_contains($v110, 'main-v110.js?v=1119')
        && str_contains($v110, 'data-hotfix-build="v110-mvp14r12-invite-notification-stability"')
        && str_contains($v110, 'Cache-Control: no-store, no-cache, must-revalidate, max-age=0'),
    'Canonical invitation launches must reach the exact current no-store production build.'
);
