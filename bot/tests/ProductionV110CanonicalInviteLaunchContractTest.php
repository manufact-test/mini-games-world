...
$assert(
    WebAppLaunchUrl::base($config) === 'https://example.test/app/v110.php?v=1113',
    'Canonical base URL must select the fresh production v1111 Telegram entrypoint.'
);
$assert(
    WebAppLaunchUrl::invitation($config, $token)
        === 'https://example.test/app/v110.php?v=1113&invite=' . $normalizedToken,
...
$assert(
    WebAppLaunchUrl::invitation($config, 'not-a-token')
        === 'https://example.test/app/v110.php?v=1113',
...
$assert(
    str_contains($v110, 'production-clean-entry-v110.js?v=1113')
        && str_contains($v110, 'main-v110.js?v=1113')
        && str_contains($v110, 'data-hotfix-build="v110-mvp14r9-mobile-share-notification-cache-root"')
