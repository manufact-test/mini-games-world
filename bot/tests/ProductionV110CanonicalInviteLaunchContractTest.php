...
$assert(
    WebAppLaunchUrl::base($config) === 'https://example.test/app/v110.php?v=1111',
    'Canonical base URL must select the fresh production v1111 Telegram entrypoint.'
);
$assert(
    WebAppLaunchUrl::invitation($config, $token)
        === 'https://example.test/app/v110.php?v=1111&invite=' . $normalizedToken,
...
$assert(
    WebAppLaunchUrl::invitation($config, 'not-a-token')
        === 'https://example.test/app/v110.php?v=1111',
