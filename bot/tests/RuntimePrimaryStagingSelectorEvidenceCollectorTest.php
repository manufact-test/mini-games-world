...
final class RuntimePrimaryStagingSelectorEvidenceCollectorTestSource implements RuntimePrimaryStagingEvidenceSourceInterface
{
...
try {
    $source = new RuntimePrimaryStagingSelectorEvidenceCollectorTestSource($projectRoot, $commit);
    $result = (new RuntimePrimaryStagingSelectorEvidenceCollector(
        $projectRoot,
        $source
    ))->collect();
    $assertTrue(($result['ok'] ?? false) === true, 'Selector-aware collector must pass');
    $assertTrue(($result['verification']['ok'] ?? false) === true, 'Selector-aware manifest must pass v3 gate');
    $assertTrue(
        ($result['manifest']['manifest_version'] ?? '') === RuntimePrimaryStagingEvidenceV3Verifier::MANIFEST_VERSION,
        'Selector-aware collector must emit v3'
    );
    $assertTrue(($result['manifest']['selector_evidence']['ready'] ?? false) === true, 'V3 manifest must contain ready selector evidence');
    $assertTrue(
        preg_match('/^[a-f0-9]{64}$/', (string)($result['selector_evidence_fingerprint'] ?? '')) === 1,
        'Selector-aware collector must expose selector evidence fingerprint'
    );
    $assertTrue($source->captureCalls === 3, 'Selector wrapper must preserve exactly three JSON evidence captures');
    $assertTrue($source->rehearsalCalls === 2, 'Selector wrapper must preserve exactly two rehearsals');
    $assertTrue(count((array)($result['projected_modules'] ?? [])) === 9, 'Selector wrapper must preserve all nine modules');

    $tampered = $result['manifest'];
    $tampered['selector_evidence']['sources']['selector'] = str_repeat('0', 64);
    $report = (new RuntimePrimaryStagingEvidenceV3Gate($projectRoot))->verify($tampered);
    $assertTrue(($report['ok'] ?? true) === false, 'Tampered selector manifest must fail v3 gate');
} finally {
    putenv('MGW_REHEARSAL_COMMIT_SHA');
}

fwrite(STDOUT, "RuntimePrimaryStagingSelectorEvidenceCollectorTest passed: {$assertions} assertions.\n");
