...
$assert(str_contains($reconciler, "api('setChatMenuButton'")
    && !str_contains($reconciler, "api('setWebhook'")
    && !str_contains($reconciler, "api('deleteWebhook'")
    && !str_contains($reconciler, "api('sendMessage'"),
    'The reconciler must mutate only the deterministic menu button setting.');
...
$assert(str_contains($reconciler, 'is_file($markerFile)')
    && str_contains($reconciler, 'file_put_contents($temporary, $identity . PHP_EOL, LOCK_EX)')
