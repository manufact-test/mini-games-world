...
assert.ok(wrapper.includes("container.dataset.mgwDominoLiveEffects = 'v33'"), 'live corrective marker must publish v33');
...
assert.ok(wrapper.includes("const PRECISION_ID = 'game-domino-effect-precision-drop'"), 'Precision v33 owner id must be explicit');
assert.ok(wrapper.includes('function mountTileLocalPrecisionV33'), 'Precision v33 corrective must exist');
...
assert.ok(wrapper.includes("local.dataset.dominoPrecisionVisual = 'single-pulse-glow-v33'"), 'Precision must publish the single-pulse glow visual');
...
assert.ok(wrapper.includes("local.innerHTML = '<i class=\"tile-aura\"></i><i class=\"tile-wave wave-1\"></i><i class=\"tile-wave wave-2\"></i>'"), 'Precision must use only two waves plus one glow aura');
...
assert.ok(liveEffectsCss.includes('@keyframes mgw-domino-precision-aura-v33'), 'Precision must include a soft tile-local glow aura');
assert.ok(!liveEffectsCss.includes('.wave-3'), 'Precision v33 must not spam a third delayed wave');

assert.ok(wrapper.includes('function correctStockBeamV33'), 'Stock v33 corrective must exist');
...
assert.ok(wrapper.includes('stock-spark-v33 p8'), 'Stock beam must include a visible multi-particle sparkle trail');
assert.ok(liveEffectsCss.includes('height:.64px'), 'Stock line must be thinner than the previous 1px beam');
assert.ok(liveEffectsCss.includes('.stock-spark-v33'), 'Stock sparkle trail must be styled');
assert.ok(liveEffectsCss.includes('@keyframes mgw-domino-stock-spark-v33'), 'Stock spark animation must exist');
...
assert.ok(entry.includes("$imports[$dominoRendererImportKey] .= '&live_effects=v33';"), 'entry must publish a fresh v33 module identity for Telegram/Hostinger');
...
assert.ok(entry.includes("header('X-MGW-Domino-Live-Effects: owner-gated-single-pulse-stock-spark-v33');"), 'entry must expose deployed v33 diagnostics');
...
console.log('MVP-19.9 Domino hand stability + live effects v33 contract: OK');
