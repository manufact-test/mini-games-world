...
if (!is_string($main) || !is_string($background) || !is_string($owner) || !is_string($prewarm) || !is_string($entry)) throw new RuntimeException('Missing v124 notification ownership source.');
$assertions = 0;
...
$assert(str_contains($entry, '"./assets/js/screens/notifications-screen-v99.js?v=99": "./assets/js/screens/notifications-passive-v124.js?v=124"')
    && str_contains($entry, '"./assets/js/first-interaction-readiness-v103.js?v=103": "./assets/js/first-interaction-readiness-v103.js?v=114"'), 'The v124 map must route notifications to passive v124 and retain prewarm.');
$assert(substr_count($entry, 'notification-window-owner-v124.js?v=124') === 1
    ...
    && !str_contains($entry, 'notification-desktop-open-owner-v117.js?v=117'), 'The active shell must publish one v124 owner and no competitors.');
$assert(str_contains($owner, "window.addEventListener('pointerdown'")
    ...
    && str_contains($owner, 'openFromUserInput();'), 'v124 must own real pointer, touch, fallback click and keyboard surfaces.');
...
$assert(str_contains($background, 'refreshNotificationBadge(false)')
    ...
    && str_contains($background, 'rememberBaselineNotifications(items, requestStartedAt)')
    && !str_contains($background, 'openNotificationsSheet('), 'The passive module may poll and show fresh toast content but cannot open the sheet.');
...
$assert(str_contains($entry, 'data-hotfix-build="v124-mvp14-d1-live-failure-fixes"')
    && str_contains($entry, 'X-MGW-Frontend-Build: v124-mvp14-d1-live-failure-fixes'), 'The shell must expose v124.');
