...
if (!is_string($entry) || !is_string($htaccess) || !is_string($main) || !is_string($residual)
    ...
    || !is_string($opponents) || !is_string($endpoint)) throw new RuntimeException('Missing v125 immutable sources.');
$assertions = 0;
...
$assert(str_contains($entry, 'type="importmap"')
    ...
    'The immutable API/session graph must remain compatible.');
$assert(str_contains($entry, '"./assets/js/screens/notifications-screen-v99.js?v=99": "./assets/js/screens/notifications-passive-v121.js?v=121"')
    ...
    'The map must route passive notifications and retain canonical invites.');
$assert(substr_count($entry, 'notification-window-owner-v121.js?v=121') === 1
    ...
    'One canonical v121 notification owner must replace every retired owner.');
$assert(substr_count($entry, 'opponents-native-fetch-v115.js?v=115') === 1
    ...
    'The player-picker transport graph must contain v122 exactly once.');
$assert(str_contains($residual, 'notificationOwner:false')
    ...
    'The residual layer must retain only request coalescing.');
$assert(str_contains($owner, "window.addEventListener('pointerdown'")
    ...
    'v121 must exclusively own real notification input and rendering.');
...
$assert(str_contains($opponents, 'REQUIRED_AUTHORITATIVE_EMPTY_RESPONSES = 3')
    ...
    'v122 must require complete JSON profile plus presence confirmation without rendering UI.');
$assert(str_contains($endpoint, 'StorageFactory::createJson(')
    ...
    'The endpoint must read canonical JSON profiles and reconcile live presence without mutation.');
...
$assert(str_contains($smoke, 'mgw_e2e_frontend=v123')
    ...
    'The live smoke must continue proving the current v123 graph until the next integration shell.');
$assert(!str_contains($entry, 'mini-games-world.com')
    && !str_contains($smoke, 'setup_secret')
    && !str_contains($smoke, 'staging_test_auth_secret'),
    'The branch must remain staging-only and secret-free.');
