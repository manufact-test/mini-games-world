...
if (!is_string($entry) || !is_string($htaccess) || !is_string($main) || !is_string($residual)
    ...
    || !is_string($opponents) || !is_string($endpoint)) throw new RuntimeException('Missing v124 immutable sources.');
$assertions = 0;
...
$assert(str_contains($entry, 'data-hotfix-build="v124-mvp14-d1-live-failure-fixes"')
    ...
    && str_contains($entry, 'Cache-Control: no-store, no-cache, must-revalidate, max-age=0'), 'The entry must expose no-cache v124 while retaining main v115.');
$assert(substr_count($entry, 'notification-window-owner-v124.js?v=124') === 1
    ...
    && !str_contains($entry, 'notification-desktop-open-owner-v117.js?v=117'), 'One canonical v124 notification owner must replace every retired owner.');
...
$assert(str_contains($owner, "window.addEventListener('pointerdown'")
    ...
    && str_contains($owner, 'openFromUserInput();'), 'v124 must exclusively own real notification input and rendering.');
...
$assert(str_contains($smoke, 'mgw_e2e_frontend=v124')
    && str_contains($smoke, "EXPECTED_BUILD = 'v124-mvp14-d1-live-failure-fixes'")
    && str_contains($smoke, "'/assets/js/screens/notification-window-owner-v124.js?v=124'")
    && str_contains($smoke, "'/assets/js/opponents-authoritative-confirm-v122.js?v=122'"), 'The live smoke must prove the integrated v124 graph.');
