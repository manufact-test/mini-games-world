...
if (!is_string($entry) || !is_string($htaccess) || !is_string($main) || !is_string($residual)
    ...
    throw new RuntimeException('Missing v119 integrated immutable-core sources.');
}
...
$assert(str_contains($entry, '"./assets/js/residual-ui-game-race-fix.js?v=91": "./assets/js/residual-ui-game-race-fix-v114.js?v=114"')
    ...
    'The immutable map must retain the proven background notification, Share and residual modules.');
...
$assert(str_contains($entry, 'data-hotfix-build="v119-mvp14-notification-canonical-owner"')
    ...
    'The entry must expose the no-cache v119 shell while retaining the proven v115 main graph.');
...
$assert(substr_count($entry, 'notification-window-owner-v119.js?v=119') === 1
    ...
    'One canonical v119 notification owner must replace every layered notification guard and responsive owner.');
...
$assert(substr_count($entry, 'opponents-native-fetch-v115.js?v=115') === 1
    ...
    'Each transport-only opponent guard must remain published exactly once.');
...
$assert(str_contains($owner, "window.addEventListener('click'")
    ...
    'The v119 owner must exclusively own notification gestures, live cache and rendering.');
...
$assert(str_contains($notifications, 'refreshNotificationBadge(false);')
    ...
    'The mapped legacy screen may retain background badge polling beneath the earlier v119 gesture boundary.');
...
$assert(str_contains($main, "import { initV115Presence } from './presence-v115.js?v=115';")
    ...
    'The integrated main entry must initialize each non-notification runtime owner exactly once.');

$assert(str_contains($smoke, 'APP_ROUTE = `${STAGING_ORIGIN}/app/?mgw_e2e_frontend=v119`')
    && str_contains($smoke, "EXPECTED_BUILD = 'v119-mvp14-notification-canonical-owner'")
    ...
    && str_contains($smoke, 'const retiredResources = [')
    && str_contains($smoke, "'/assets/js/presence-v115.js?v=115'"),
    'The live smoke test must prove the v119 shell, canonical notification owner and absence of retired notification resources.');
