...
final class ProductionPrimaryApplicationEntrypoints
{
    ...
    public static function resolve(string $projectRoot, array $server): string
    {
        ...
        if (in_array($relative, ['bot/admin-read.php', 'bot/admin-economy.php', 'bot/admin-rating.php'], true)) {
            // Web Admin surfaces are not separate storage/runtime owners. The
            // accepted read-only snapshot, the narrow MVP-15.8 economy mutation
            // endpoint and MVP-20.8 rating operations reuse the existing API
            // DB-primary context.
            return 'api';
        }
