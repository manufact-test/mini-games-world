...
final class CosmeticStoreService
{
    ...
    public function snapshot(string $mgwId, int $balance, array $coinPackages = []): array
    {
        ...
        $offers = [];
        foreach ($this->activeOffers() as $row) {
            $offer = $this->publicOffer($row, $owned, $balance);
        ...
        $bundle = $offers[self::AVATAR_BUNDLE_OFFER_ID] ?? null;
        if (is_array($bundle)) {
            $bundle['preview_kind'] = 'avatar_bundle_placeholder';
        }
        ...
        foreach ((array)($inventory['owned'] ?? []) as $ownedRow) {
            ...
            if ((string)($catalog['item_family'] ?? '') !== 'avatar') continue;
            ...
            $ownedItems[] = [
                ...
                'item_family' => 'avatar',
                'equip_slot' => (string)($catalog['equip_slot'] ?? ProductInventoryService::PROFILE_AVATAR_SLOT),
                'starter' => $starterIndex !== false,
                'store_product' => $storeIndex !== false,
                'preview_number' => $starterIndex !== false ? $starterIndex + 1 : ($storeIndex !== false ? $storeIndex + 4 : null),
                'preview_kind' => 'numbered_avatar_placeholder',
                'equipped' => (string)($equipped[ProductInventoryService::PROFILE_AVATAR_SLOT] ?? '') === $itemId,
        ...
        usort($ownedItems, static function (array $left, array $right): int {
            return ((int)($left['preview_number'] ?? 999)) <=> ((int)($right['preview_number'] ?? 999));
        });
        ...
        return [
            ...
            'tabs' => [
                ...
                ['id' => 'games', 'label' => 'Игры', 'available' => false],
                ['id' => 'bundles', 'label' => 'Наборы', 'available' => true],
                ['id' => 'inventory', 'label' => 'Инвентарь', 'available' => true],
                ['id' => 'tournament_rewards', 'label' => 'Турнирные награды', 'available' => false],
            ],
            'coins' => [
                'packages' => $packages,
                'billing_available' => false,
            ...
            'games' => [
                'available' => false,
                'message' => 'Игровая косметика появится после общего cosmetics framework.',
            ],
            'bundles' => [
                'avatar_bundle' => $bundle,
            ],
            'inventory' => [
                'items' => $ownedItems,
                'equipped' => $equipped,
            ...
            'tournament_rewards' => [
                'available' => false,
                'message' => 'Турнирные награды появятся вместе с турнирами.',
            ],
            'purchase_rules' => [
                'auto_equip' => false,
    ...
    public function quote(string $mgwId, string $offerId): array
    {
        ...
        if ((string)$offer['offer_type'] === 'item') {
            ...
            $price = (int)$offer['price_coins'];
        } else {
            $partial = (int)($offer['partial_unit_price_coins'] ?? 0);
            if ($partial <= 0) throw new CosmeticStoreException('offer_invalid', 'Цена набора недоступна.');
            $price = count($missing) === count($members)
                ? (int)$offer['price_coins']
                : $partial * count($missing);
        }
    ...
    public function fulfill(
        ...
        return $this->database->transaction(function (DatabaseConnectionInterface $database) use (
            ...
            $expectedPrice = (string)$offer['offer_type'] === 'item'
                ? (int)$offer['price_coins']
                : (count($itemIds) === count($members)
                    ? (int)$offer['price_coins']
                    : (int)($offer['partial_unit_price_coins'] ?? 0) * count($itemIds));
    ...
    private function activeOffers(): array
    {
    ...
    private function publicOffer(array $row, array $owned, int $balance): array
    {
        ...
        $price = $type === 'bundle' && $missing !== [] && count($missing) < count($members)
            ? (int)$partial * count($missing)
            : $fullPrice;
