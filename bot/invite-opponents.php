...
function mgw_invite_opponents_storage(array $config): StorageAdapterInterface
{
    ...
    if ($environment === 'staging') {
        ...
        return new DatabasePrimaryStateStorageAdapter(PdoConnectionFactory::create($databaseConfig), null);
    }
...
try {
    ...
    $items = $storage->readOnly(function (array $data) use ($userId, $onlineIds): array {
        ...
        usort($result, static function (array $left, array $right): int {
            ...
            return $scoreCompare !== 0 ? $scoreCompare : strcasecmp((string)($left['name'] ?? ''), (string)($right['name'] ?? ''));
        });
