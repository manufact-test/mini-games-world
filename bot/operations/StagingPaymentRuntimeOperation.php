...
final class StagingPaymentRuntimeOperation
{
    private const BUILD = 'v99-mvp14-db-payment-routing';
    private const OPERATION_ID = 'mvp-14.8.4h-payment-runtime-v1';
    ...
    public function __construct(
        ...
        if ($this->privateDir === '' || !is_dir($this->privateDir)) {
            throw new RuntimeException('Payment runtime operation private directory is unavailable.');
        }
    }
    ...
    public function definition(): StagingOperationDefinition
    {
        return new StagingOperationDefinition(
            ...
            function (): array {
                ...
                $report['operation_type'] = 'payment_runtime_activation';
    ...
    private function controller(): RuntimeModuleActivationController
    {
        ...
        $repository = function (array $runtime) use ($configForRuntime): RuntimePaymentRepository {
        ...
        $synchronize = function (array $runtime) use ($repository): array {
            $schema = (new RuntimePaymentSchemaInstaller($this->database))->install();
            $result = $repository($runtime)->bootstrapCurrentJson();
            $result['schema'] = $schema;
            return $result;
        };
        $audit = function (array $runtime) use ($repository): array {
            $schema = (new RuntimePaymentSchemaInstaller($this->database))->verify();
            $result = $repository($runtime)->auditParity();
            if (empty($schema['ok'])) {
                $result['ok'] = false;
                $result['blockers'][] = 'Payment runtime schema verification failed.';
            }
            $result['schema'] = $schema;
            return $result;
        };
        ...
        return new RuntimeModuleActivationController(
            ...
            $this->privateDir . '/payment-runtime-activation-v1.json',
            $this->privateDir . '/payment-runtime-activation-v1.runtime.backup',
            $synchronize,
            $audit,
            'mvp-14.8.4h-payment-runtime-activation'
        );
    }
}
