...
async function openPlayer(browser, slot, { isMobile = true } = {}) {
...
test('D1 v120 acceptance: declined invitation remains visible read history without actions or toast', async ({ browser }) => {
  let playerA;
  let playerB;
  try {
    ...
    }, 'Create v120 decline-history invitation');
    ...
    await playerA.page.locator('#notificationToast').click();
    ...
    expectClean(playerA, 'Player A v120 declined history');
    expectClean(playerB, 'Player B v120 declined history');
  } finally {
...
test('D1 v120 acceptance: mobile notification toast paints cached invitation before delayed false-empty response', async ({ browser }) => {
  let playerA;
  let playerB;
  try {
    playerA = await openPlayer(browser, 'A');
    ...
    const created = await expectPost(playerB.page, '/bot/invites.php', {
      action:'create_direct', inviteeId:'stg_test_player_a', gameType:'tictactoe',
      room:'match', bet:10, boardSize:3,
    }, 'Create v120 mobile delayed-response invitation');
    const token = String(created?.invite?.token || '');
    expect(token).toMatch(/^[A-Za-z0-9_-]{12,80}$/);
    const toast = playerA.page.locator('#notificationToast');
    await expect(toast).toHaveClass(/show/, { timeout:20_000 });
    ...
    await recordSheetTrace(playerA.page, '__MGW_D1_V120_MOBILE_DELAY');
    ...
    expectClean(playerA, 'Player A v120 mobile delayed response');
    expectClean(playerB, 'Player B v120 mobile delayed response');
  } finally {
    if (playerA) await cleanupPlayer(playerA);
    if (playerB) await cleanupPlayer(playerB);
    await closePlayer(playerA);
...
test('D1 v120 acceptance: desktop bell remains immediately reusable while a prior request finishes', async ({ browser }) => {
  let playerA;
  try {
    ...
    await playerA.page.unroute(NOTIFICATIONS_ROUTE);
    expectClean(playerA, 'Player A v120 desktop in-flight bell');
  } finally {
    if (playerA) await cleanupPlayer(playerA);
    await closePlayer(playerA);
  }
