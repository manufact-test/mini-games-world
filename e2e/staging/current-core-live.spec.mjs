...
const AUTH = `${ORIGIN}/bot/staging-test-auth.php`;
...
async function oidcToken() {
  const urlValue = process.env.ACTIONS_ID_TOKEN_REQUEST_URL || '';
  const requestToken = process.env.ACTIONS_ID_TOKEN_REQUEST_TOKEN || '';
  if (!urlValue || !requestToken) throw new Error('GitHub Actions OIDC environment is unavailable.');
...
function diagnostics(page, slot) {
  const value = { slot, pageErrors: [], serverErrors: [], failedRequests: [], presenceStatuses: [] };
  ...
  page.on('requestfailed', request => {
    ...
    value.failedRequests.push({ path: new URL(request.url()).pathname, error: request.failure()?.errorText || '' });
  });
  return value;
}
...
async function ok(page, path, data, label) {
  const result = await post(page, path, data);
...
async function openPlayer(browser, slot) {
  ...
  const bootstrapPromise = page.waitForResponse(response => (
    ...
    && actionOf(response.request()) === 'bootstrap'
  ), { timeout: 35_000 });

  const entry = await page.goto(ENTRY_URL, { waitUntil: 'domcontentloaded' });
  expect(entry?.ok(), `Player ${slot} active Telegram entry`).toBe(true);
  ...
  const profile = await ok(page, '/bot/api.php', { action: 'profile' }, `Player ${slot} profile`);
...
async function waitInviteEvent(player, token) {
  ...
  await expect.poll(async () => {
    const result = await post(player.page, '/bot/invites.php', { action: 'sync' });
    ...
    message: 'Current v110 invite_events owner receives the direct invitation',
    timeout: 8_000,
...
async function gameState(player, gameId) {
  return ok(player.page, '/bot/api.php', { action: 'game_state', gameId }, 'game state');
}

async function waitLaunch(players, gameId) {
  let game = null;
  await expect.poll(async () => {
    const result = await post(players[0].page, '/bot/api.php', { action: 'game_state', gameId });
    if (result.status !== 200 || result.payload?.ok !== true) return `http:${result.status}`;
    const candidate = result.payload?.game || {};
    const ready = candidate.status === 'active'
    ...
    return ready ? 'ready' : `${candidate.launch_phase || ''}:${candidate.turn || ''}`;
  }, { timeout: 20_000, intervals: [100, 200, 400, 800] }).toBe('ready');
  return game;
}

async function waitTurn(player, gameId) {
  const id = player.profile.user.id;
  await expect.poll(async () => {
    const result = await post(player.page, '/bot/api.php', { action: 'game_state', gameId });
...
async function firstUiTap(player, gameId, cell) {
  ...
  const responsePromise = player.page.waitForResponse(response => (
    response.url() === `${ORIGIN}/bot/api.php`
    && actionOf(response.request()) === 'game_action'
  ), { timeout: 20_000 });
  await button.click();
...
async function directCell(player, gameId, cell) {
  await waitTurn(player, gameId);
  const result = await ok(player.page, '/bot/api.php', {
...
async function historyItem(player, gameId) {
  const result = await post(player.page, '/bot/api.php', { action: 'history' });
...
test('CURRENT LIVE CORE: canonical Telegram v110 two-player TTT lifecycle', async ({ browser }) => {
  let A;
  let B;
  try {
    ...
    const created = await ok(A.page, '/bot/invites.php', {
    ...
    const accepted = await ok(B.page, '/bot/invites.php', { action: 'accept', token }, 'accept invite');
    expect(accepted.invite?.status).toBe('accepted');
    expect(accepted.invite?.can_start).toBe(false);

    const started = await ok(A.page, '/bot/invites.php', { action: 'start', token }, 'start invite');
    ...
    let game = await waitLaunch([A, B], gameId);
    ...
    const afterA = await ok(A.page, '/bot/api.php', { action: 'profile' }, 'A final profile');
    const afterB = await ok(B.page, '/bot/api.php', { action: 'profile' }, 'B final profile');
    const after = {
    ...
    for (const player of [A, B]) {
      ...
      await player.context.close().catch(() => null);
    }
    await resetPlayers();
  }
});
