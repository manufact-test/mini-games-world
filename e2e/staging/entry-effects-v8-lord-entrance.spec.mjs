...
const MODULE='/app/assets/js/entry-effects/mgw-entry-effects-v8-lord-entrance.js?v=4';
...
test('Entry 03 Lord Entrance mounts premium portal, detailed armor, open-face lord, actual sword motion and slash layers',async({page},testInfo)=>{
  ...
  await page.waitForFunction(()=>{const body=document.querySelector('.mgw-entry-v8-le-body');const face=document.querySelector('.mgw-entry-v8-le-face');const sword=document.querySelector('.mgw-entry-v8-le-sword');return [body,face,sword].every(el=>el instanceof HTMLImageElement&&el.complete&&el.naturalWidth>0);});
  await expect(page.locator('#fixture')).toHaveAttribute('data-entry-v8-lord-entrance','1');
  for(const selector of ['.mgw-entry-v8-le-portal','.mgw-entry-v8-le-lord','.mgw-entry-v8-le-body','.mgw-entry-v8-le-face','.mgw-entry-v8-le-sword-wrap','.mgw-entry-v8-le-slash','.mgw-entry-v8-le-shock','.mgw-entry-v8-le-cloth','.mgw-entry-v8-le-cape']) await expect(page.locator(selector)).toHaveCount(1);
  ...
  const state=await page.evaluate(()=>({
    ...
    face:getComputedStyle(document.querySelector('.mgw-entry-v8-le-face')).animationName,
    ...
    faceSrc:document.querySelector('.mgw-entry-v8-le-face')?.getAttribute('src')||'',
    skip:document.getElementById('skip')?.textContent,
  }));
  expect(state.oldArt).toBe('none');expect(state.oldFx).toBe('none');
  expect(state.lord).toContain('mgwLeLord');expect(state.face).toContain('mgwLeFace');expect(state.sword).toContain('mgwLeSword');expect(state.slash).toContain('mgwLeSlash');expect(state.stage).toContain('mgwLeStage');expect(state.skip).toBe('Пропустить');
  expect(state.bodySrc).toContain('entry-03-lord-entrance-armor-detail.webp');
  expect(state.bodyWidth).toBeGreaterThanOrEqual(400);expect(state.bodyHeight).toBeGreaterThanOrEqual(400);
  expect(state.faceSrc).toContain('entry-03-lord-entrance-open-face.svg');
...
test('Lord Entrance reduced motion keeps coherent open-face lord and portal but removes slash/shake',async({browser})=>{
  const context=await browser.newContext({viewport:{width:390,height:844},reducedMotion:'reduce'});try{const page=await context.newPage();await mountFixture(page,'entry-03');await page.waitForSelector('.mgw-entry-v8-lord-entrance');await page.waitForFunction(()=>{const b=document.querySelector('.mgw-entry-v8-le-body');const f=document.querySelector('.mgw-entry-v8-le-face');return [b,f].every(el=>el instanceof HTMLImageElement&&el.complete&&el.naturalWidth>0);});const s=await page.evaluate(()=>({lord:getComputedStyle(document.querySelector('.mgw-entry-v8-le-lord')).opacity,face:getComputedStyle(document.querySelector('.mgw-entry-v8-le-face')).opacity,portal:getComputedStyle(document.querySelector('.mgw-entry-v8-le-portal')).opacity,stageAnim:getComputedStyle(document.querySelector('.mgw-entry-v8-le-stage')).animationName,slash:getComputedStyle(document.querySelector('.mgw-entry-v8-le-slash')).display,sword:getComputedStyle(document.querySelector('.mgw-entry-v8-le-sword-wrap')).display}));expect(s.lord).toBe('1');expect(s.face).toBe('1');expect(Number(s.portal)).toBeGreaterThan(0);expect(s.stageAnim).toBe('none');expect(s.slash).toBe('none');expect(s.sword).toBe('none');}finally{await context.close();}
});
