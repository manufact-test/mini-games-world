...
const APP_ROUTE = `${STAGING_ORIGIN}/app/?mgw_e2e_frontend=v122`;
const API_ROUTE = `${STAGING_ORIGIN}/bot/api.php`;
const TEST_COOKIE = 'mgw_staging_test_session';
const EXPECTED_BUILD = 'v122-mvp14-opponents-authoritative-source';
...
test('staging app root serves v119 notifications with authoritative opponent v122', async ({ browser }) => {
  ...
  try {
    ...
    for (const suffix of required) {
      expect(graph.resources.some(url => new URL(url).pathname.concat(new URL(url).search).endsWith(suffix)),
        `Fresh graph must include ${suffix}`).toBe(true);
    }
    ...
    for (const suffix of retired) {
      expect(graph.resources.some(url => new URL(url).pathname.concat(new URL(url).search).endsWith(suffix)),
        `Retired graph must exclude ${suffix}`).toBe(false);
    }
