...
const APP_ROUTE = `${STAGING_ORIGIN}/app/?mgw_e2e_frontend=v121`;
const API_ROUTE = `${STAGING_ORIGIN}/bot/api.php`;
const TEST_COOKIE = 'mgw_staging_test_session';
const EXPECTED_BUILD = 'v121-mvp14-notification-short-input-owner';
...
async function revokeContext(context) {
  try { await context.request.post(AUTH_ROUTE, { data:{ action:'revoke' }, timeout:15_000 }); }
  catch {}
}

test('staging app root serves one canonical v121 notification input graph', async ({ browser }) => {
  ...
  try {
    ...
    const fatalModuleError = new Promise((_, reject) => {
      page.once('pageerror', error => reject(new Error(`Frontend module graph failed before bootstrap: ${error?.message || error}`)));
    });
    ...
    await page.waitForFunction(() => window.__MGW_FIRST_INTERACTION_READY__ !== undefined, null, { timeout:25_000 });
    ...
    for (const suffix of requiredFreshResources) {
      expect(graph.resources.some(url => new URL(url).pathname.concat(new URL(url).search).endsWith(suffix)),
        `Fresh module graph must include ${suffix}`).toBe(true);
    }
    ...
    for (const suffix of retiredResources) {
      expect(graph.resources.some(url => new URL(url).pathname.concat(new URL(url).search).endsWith(suffix)),
        `Retired notification graph must exclude ${suffix}`).toBe(false);
    }
